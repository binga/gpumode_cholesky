...
"""GPU MODE `cholesky` submission — experiment 015 candidate A round 4.
...
if _HAVE_TRITON:
    ...
    def _micro_potrf_gj32(
        ...
        for p in range(0, 32):
            ...
            inv = 1.0 / tl.sqrt(dpp)
            colp = tl.where(r >= p, colp * inv, 0.0)
            a = tl.where(c[None, :] == p, colp[:, None], a)
            trail = (r[:, None] > p) & (c[None, :] > p)
            a = tl.where(trail, a - colp[:, None] * colp[None, :], a)
            # Row p of the factor is now final: L[p, :p] came from earlier
            # columns and L[p, p] = sqrt(dpp) = dpp * inv. Build inverse row p.
            rowp = tl.sum(tl.where(r[:, None] == p, a, 0.0), axis=0)
            lpp = dpp * inv
            rm = tl.where(c < p, rowp, 0.0)
            contrib = tl.sum(rm[:, None] * x, axis=0)
            eq = tl.where(c == p, 1.0, 0.0)
            x = tl.where(r[:, None] == p, ((eq - contrib) / lpp)[None, :], x)
