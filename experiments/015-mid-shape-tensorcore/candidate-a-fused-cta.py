...
"""GPU MODE `cholesky` submission — experiment 015 candidate A.

Adds a fused one-CTA-per-matrix blocked tensor-core factorization for seven
mid shapes (64x256, 16x512, 640x512, 4x1024, 60x1024, 2x2048, 8x2048) on top
of the exact exp-014 ranked winner (#880770). Everything below this paragraph
is the unchanged exp-014 module documentation.
...
if _HAVE_TRITON:
    ...
    def _fused_potrf_cta(
        ...
        """One CTA factorizes one n x n SPD matrix, blocked right-looking.

        Reads the input matrix from a_ptr and writes the lower factor to l_ptr
        out-of-place (the strict upper triangle is written as exact zeros), so
        no clone or upper-clear pass is needed. The BK x BK diagonal block is
        factorized serially in registers, its triangular inverse is built with
        BK serial substitution steps, and the panel and trailing updates run on
        tensor cores via tl.dot (TF32 inputs, FP32 accumulation).
        """
        pid = tl.program_id(0).to(tl.int64)
        a_base = a_ptr + pid * n * n
        l_base = l_ptr + pid * n * n
        rows = tl.arange(0, BK)
        cols = tl.arange(0, BK)
        ...
        for k in range(0, NT):
            koff = k * BK
            d_off = (koff + rows[:, None]) * n + (koff + cols[None, :])
            if k == 0:
                d = tl.load(a_base + d_off)
            else:
                d = tl.load(l_base + d_off)

            # Serial Cholesky of the diagonal block (FP32, in registers).
            for p in range(0, BK):
                dpp = tl.sum(
                    tl.where(
                        (rows[:, None] == p) & (cols[None, :] == p), d, 0.0
                    )
                )
                inv = 1.0 / tl.sqrt(dpp)
                cmask = (cols[None, :] == p) & (rows[:, None] >= p)
                d = tl.where(cmask, d * inv, d)
                colp = tl.sum(tl.where(cols[None, :] == p, d, 0.0), axis=1)
                trail = (rows[:, None] > p) & (cols[None, :] > p)
                d = tl.where(trail, d - colp[:, None] * colp[None, :], d)
            d = tl.where(lower, d, 0.0)
            tl.store(l_base + d_off, d)

            if k + 1 < NT:
                # Triangular inverse of d by forward substitution, one row of
                # X = D^-1 per step: X[q,:] = (I[q,:] - D[q,:q] @ X[:q,:]) / dqq.
                x = tl.where(rows[:, None] == cols[None, :], 1.0, 0.0)
                for q in range(0, BK):
                    rowq = tl.sum(
                        tl.where(rows[:, None] == q, d, 0.0), axis=0
                    )
                    dqq = tl.sum(tl.where(cols == q, rowq, 0.0), axis=0)
                    rmask = tl.where(cols < q, rowq, 0.0)
                    contrib = tl.sum(rmask[:, None] * x, axis=0)
                    eq = tl.where(cols == q, 1.0, 0.0)
                    newrow = (eq - contrib) / dqq
                    x = tl.where(rows[:, None] == q, newrow[None, :], x)
                xt = tl.trans(x)

                # Panel: L[i,k] = A[i,k] @ D^-T on tensor cores; the mirrored
                # (k,i) tile is zero-filled so the output is exactly lower
                # triangular without a separate clear pass.
                zeros = tl.zeros((BK, BK), dtype=tl.float32)
                for i in range(k + 1, NT):
                    ioff = i * BK
                    p_off = (ioff + rows[:, None]) * n + (koff + cols[None, :])
                    if k == 0:
                        p_tile = tl.load(a_base + p_off)
                    else:
                        p_tile = tl.load(l_base + p_off)
                    lik = tl.dot(
                        p_tile,
                        xt,
                    ...
                    tl.store(l_base + p_off, lik)
                    m_off = (koff + rows[:, None]) * n + (ioff + cols[None, :])
                    tl.store(l_base + m_off, zeros)
                tl.debug_barrier()

                # Trailing Schur update of the lower-triangular tile region:
                # T[i,j] -= L[i,k] @ L[j,k]^T for k < j <= i < NT.
                for i in range(k + 1, NT):
                    ioff = i * BK
                    li = tl.load(
                        l_base
                    ...
                    for j in range(k + 1, i + 1):
                        ...
                        if k == 0:
                            t = tl.load(a_base + t_off)
                        else:
                            t = tl.load(l_base + t_off)
                        tl.store(l_base + t_off, t - prod)
                tl.debug_barrier()

    # (batch, n) -> (BK, num_warps) for the fused one-CTA-per-matrix path.
    _FUSED_CTA_SHAPES = {
        (64, 256): (64, 8),
        (16, 512): (64, 8),
        (640, 512): (64, 8),
    ...
    def _fused_potrf(data: torch.Tensor) -> torch.Tensor:
        batch, n, _ = data.shape
        bk, warps = _FUSED_CTA_SHAPES[(batch, n)]
        data = data.contiguous()
        out = torch.empty_like(data)
        _fused_potrf_cta[(batch,)](
            data,
            out,
            n=n,
            NT=n // bk,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Experiment 015: fused one-CTA-per-matrix blocked tensor-core potrf for
    # the mid shapes. On any numerical failure (non-finite diagonal from TF32
    # on ill-conditioned families) fall through to the previously shipped
    # dispatch below, which remains the exact ranked behavior.
    if is_f32_cuda and _HAVE_TRITON and (batch, n) in _FUSED_CTA_SHAPES:
        try:
            l = _fused_potrf(data)
