...
"""GPU MODE `cholesky` submission — experiment 015 candidate B.

Adds, on top of the exact exp-014 ranked winner (#880770), exactly one new
path: a graph-replayed exact cuSOLVER factorization for (1024, 64) (static
input buffer, replay, owned clone out) measuring 1.08x paired. Explored and
REJECTED on B200 measurements (r1-r3 probes): fused Triton one-CTA diagonal
blocks for a blocked (1,4096)/(2,4096) superpanel (0.18-0.33x), the same with
captured cuSOLVER diagonal blocks (0.52-0.58x), whole-call graph capture at
4096 (0.97x — cuSOLVER's 4096 potrf is compute-busy, not launch-bound), and a
fused Triton (1024,64) factorization (0.67x). Everything below this paragraph
...
# ---------------------------------------------------------------------------
# Experiment 015 candidate B: graph-replayed cuSOLVER-free superpanel path for
# (1,4096) and (2,4096), and a fused Triton path for (1024,64).
# ---------------------------------------------------------------------------
_GRAPH_SP_HITS = 0
...
def _graph_cholesky_1024x64(data: torch.Tensor) -> torch.Tensor | None:
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Experiment 015 candidate B paths. Each returns None on any failure so
    # the previously shipped dispatch below remains the fallback behavior.
