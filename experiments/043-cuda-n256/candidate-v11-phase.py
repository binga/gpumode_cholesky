...
__device__ __forceinline__ unsigned long long global_nanoseconds256() {
...
void cholesky256_wmma16(
    ...
    if (tid == 0) launch_start = global_nanoseconds256();
    __syncthreads();
    ...
    for (int block = 0; block < N256; block += BK256) {
        ...
        for (int local = 0; local < BK256; ++local) {
            ...
            if (tid < BK256 * BK256
                && row > pivot && column > pivot && column <= row) {
        ...
        if (tid == 0) {
            const unsigned long long now = global_nanoseconds256();
    ...
    __syncthreads();
    if (tid == 0) {
        const unsigned long long launch_end = global_nanoseconds256();
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA256 = load_inline(
            name="chol256_exp043_v11_wmma_phase",
            cpp_sources=(
                "void chol256_launch("
...
def _cuda_cholesky256(data: torch.Tensor) -> torch.Tensor:
    ...
    _CUDA256.chol256_launch(data, out, timings)
    _CUDA256_HITS += 1
    return out
