...
void cholesky256_wmma16(const float* input, float* output) {
    ...
    for (int block = 0; block < N256; block += BK256) {
        ...
        const int diagonal_row = tid;
        float diagonal_values[BK256];
        if (diagonal_row < BK256) {
            #pragma unroll
            for (int column = 0; column < BK256; ++column) {
                diagonal_values[column] = column <= diagonal_row
                    ? tile[tile_index256(block + diagonal_row, block + column)]
                    : 0.0f;
            }
        }
        ...
        for (int iteration = 0; iteration < BK256 / 2; ++iteration) {
            ...
            if (diagonal_row == q) {
                diagonal_values[q] = fmaf(
                    -diagonal_values[k], diagonal_values[k], diagonal_values[q]);
            ...
            if (diagonal_row < BK256 && diagonal_row >= q) {
                if (diagonal_row != q) {
                    diagonal_values[q] = fmaf(
                        -diagonal_values[k], pivot0[q], diagonal_values[q]);
                }
            ...
            if (diagonal_row < BK256 && diagonal_row > q) {
                ...
                for (int column = q + 1; column < BK256; ++column) {
                    if (column <= diagonal_row) {
                        float value = fmaf(
                            -scale0, pivot0[column], diagonal_values[column]);
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA256 = load_inline(
            name="chol256_exp043_v12_rank2diag_wmma",
