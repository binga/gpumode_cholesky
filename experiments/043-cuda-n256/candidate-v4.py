...
#
# A full padded 256x256 FP32 tile exceeds one CTA's shared-memory budget. The
# lower 16x16 tiles fit when packed at tile granularity; a 17-float row stride
# removes the bank conflicts of row-wise triangular packing. One 32-warp CTA
# keeps the complete blocked-16 factorization in that shared tile collection,
# replacing the ranked path's 30-operation split32 graph and host-visible gate.
# ---------------------------------------------------------------------------
_CUDA256_HITS = 0
_CUDA256_ERROR = None
_CUDA256 = None
...
__device__ __forceinline__ int lower_index256(int row, int column) {
...
void cholesky256_packed_block16(const float* input, float* output) {
    const int tid = threadIdx.x;
    const int warp = tid >> 5;
    const int lane = tid & 31;
    ...
    // Full-linear traversal keeps global accesses coalesced; only lower values
    ...
    for (int linear = tid; linear < N256 * N256; linear += THREADS256) {
        ...
        if (column <= row) {
            tile[lower_index256(row, column)] = input[base + linear];
        }
    }
    __syncthreads();
    ...
    for (int block = 0; block < N256; block += BK256) {
        ...
        if (warp == 0) {
            ...
            for (int local = 0; local < BK256; ++local) {
                ...
                float inverse = 0.0f;
                if (lane == local) {
                    const int diagonal = lower_index256(pivot, pivot);
                    inverse = rsqrtf(tile[diagonal]);
                    inverse_diag[local] = inverse;
                    tile[diagonal] *= inverse;
                }
                inverse = __shfl_sync(0xffffffffu, inverse, local);
                if (lane > local && lane < BK256) {
                    tile[lower_index256(block + lane, pivot)] *= inverse;
                }
                __syncwarp();
                if (lane > local && lane < BK256) {
                    const int row = block + lane;
                    const float row_pivot = tile[lower_index256(row, pivot)];
                    #pragma unroll
                    for (int column_local = local + 1;
                         column_local <= lane;
                         ++column_local) {
                        const int column = block + column_local;
                        const int offset = lower_index256(row, column);
                        tile[offset] = fmaf(
                            -row_pivot,
                            tile[lower_index256(column, pivot)],
                            tile[offset]);
                    }
                }
                __syncwarp();
        ...
        const int row = block_end + tid;
        if (row < N256) {
            const int row_block_base = lower_index256(row, block);
            #pragma unroll
            for (int local = 0; local < BK256; ++local) {
                const int column = block + local;
                float value = tile[row_block_base + local];
                const int column_block_base = lower_index256(column, block);
                #pragma unroll
                for (int prior = 0; prior < local; ++prior) {
                    value = fmaf(
        ...
        for (int trailing_row = block_end + warp;
            ...
            const int row_block_base = lower_index256(trailing_row, block);
            for (int column = block_end + lane;
                 column <= trailing_row;
                 column += 32) {
                const int column_block_base = lower_index256(column, block);
                ...
                for (int k = 0; k < BK256; ++k) {
                    update = fmaf(
                        ...
                        tile[column_block_base + k],
                        update);
                }
                tile[lower_index256(trailing_row, column)] -= update;
            }
        }
        __syncthreads();
    }
    ...
    for (int linear = tid; linear < N256 * N256; linear += THREADS256) {
        ...
        const int column = linear & 255;
        output[base + linear] =
            column <= row ? tile[lower_index256(row, column)] : 0.0f;
    }
}
...
void chol256_launch(torch::Tensor input, torch::Tensor output) {
    ...
    const int batch = (int)input.size(0);
    cholesky256_packed_block16<<<dim3(batch), dim3(THREADS256), SHARED_BYTES256>>>(
        input.data_ptr<float>(), output.data_ptr<float>());
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
}
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA256 = load_inline(
            name="chol256_exp043_v4_warpdiag_tiled17_t1024",
