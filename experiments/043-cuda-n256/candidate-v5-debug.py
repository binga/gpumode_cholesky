...
#
# A two-block CUDA cluster owns each matrix. Alternating 16-row tile bands are
# held in each CTA's distributed shared memory, with a padded 17-float stride.
# This gives the dominant trailing update two SMs while keeping all blocked-16
# dependencies inside one launch. Across 64 matrices the 128 CTAs fit in one
# B200 scheduling wave.
# ---------------------------------------------------------------------------
_CUDA256_HITS = 0
_CUDA256_ERROR = None
_CUDA256 = None
...
constexpr int THREADS256 = 512;
constexpr int CLUSTER_BLOCKS256 = 2;
constexpr int TILE_STRIDE256 = 17;
constexpr int TILE_VALUES256 = BK256 * TILE_STRIDE256;
constexpr int MAX_LOCAL_TILES256 = 72;
constexpr int TILE_FLOATS256 = MAX_LOCAL_TILES256 * TILE_VALUES256;
constexpr int SHARED_FLOATS256 = TILE_FLOATS256 + BK256 + 1;
constexpr int SHARED_BYTES256 = SHARED_FLOATS256 * sizeof(float);

__device__ __forceinline__ int local_index256(int row, int column) {
    const int tile_row = row >> 4;
    const int tile_column = column >> 4;
    const int pair_row = tile_row >> 1;
    const int preceding = (tile_row & 1)
        ? pair_row * (pair_row + 1)
        : pair_row * pair_row;
    const int tile = preceding + tile_column;
    return tile * TILE_VALUES256 + (row & 15) * TILE_STRIDE256 + (column & 15);
}

__global__ __launch_bounds__(THREADS256, 1)
void cholesky256_cluster2(const float* input, float* output) {
    ...
    const int matrix = (int)blockIdx.x / CLUSTER_BLOCKS256;
    const size_t base = (size_t)matrix * N256 * N256;
    extern __shared__ float tile[];
    float* tile0 = cluster.map_shared_rank(tile, 0);
    float* tile1 = cluster.map_shared_rank(tile, 1);

    for (int linear = tid; linear < N256 * N256; linear += THREADS256) {
        const int row = linear >> 8;
        const int column = linear & 255;
        if (column <= row && ((row >> 4) & 1) == rank) {
            tile[local_index256(row, column)] = input[base + linear];
        }
    }
    cluster.sync();
    ...
    for (int block = 0; block < N256; block += BK256) {
        ...
        const int diagonal_owner = (block >> 4) & 1;
        float* diagonal_tile = diagonal_owner ? tile1 : tile0;
        float* inverse_diag = diagonal_tile + TILE_FLOATS256;
        ...
        if (rank == diagonal_owner) {
            ...
            for (int local = 0; local < BK256; ++local) {
                const int pivot = block + local;
                if (tid == 0) {
                    const int diagonal = local_index256(pivot, pivot);
                    const float inverse = rsqrtf(tile[diagonal]);
                    inverse_diag[local] = inverse;
                    tile[TILE_FLOATS256 + BK256] = inverse;
                    tile[diagonal] *= inverse;
                }
                __syncthreads();

                const int panel_row = pivot + 1 + tid;
                if (panel_row < block_end) {
                    tile[local_index256(panel_row, pivot)] *=
                        tile[TILE_FLOATS256 + BK256];
                }
                __syncthreads();

                for (int linear = tid; linear < BK256 * BK256;
                     linear += THREADS256) {
                    const int row = block + (linear >> 4);
                    const int column = block + (linear & 15);
                    if (row > pivot && column > pivot && column <= row) {
                        const int offset = local_index256(row, column);
                        tile[offset] = fmaf(
                            -tile[local_index256(row, pivot)],
                            tile[local_index256(column, pivot)],
                            tile[offset]);
                    }
                }
                __syncthreads();
        ...
        const int row = block_end + tid;
        if (row < N256 && ((row >> 4) & 1) == rank) {
            const int row_block_base = local_index256(row, block);
            #pragma unroll
            for (int local = 0; local < BK256; ++local) {
                const int column = block + local;
                float value = tile[row_block_base + local];
                const int column_block_base = local_index256(column, block);
                #pragma unroll
                for (int prior = 0; prior < local; ++prior) {
                    value = fmaf(
                        -tile[row_block_base + prior],
                        diagonal_tile[column_block_base + prior],
                        value);
                }
        ...
        for (int trailing_row = block_end + warp;
             trailing_row < N256;
             trailing_row += THREADS256 / 32) {
            if (((trailing_row >> 4) & 1) != rank) continue;
            const int row_block_base = local_index256(trailing_row, block);
            for (int column = block_end + lane;
                 column <= trailing_row;
                 column += 32) {
                float* column_tile = ((column >> 4) & 1) ? tile1 : tile0;
                const int column_block_base = local_index256(column, block);
                float update = 0.0f;
                #pragma unroll
                for (int k = 0; k < BK256; ++k) {
                    update = fmaf(
                        tile[row_block_base + k],
                        column_tile[column_block_base + k],
                        update);
                }
                tile[local_index256(trailing_row, column)] -= update;
            }
        }
        cluster.sync();
    }
    ...
    for (int linear = tid; linear < N256 * N256; linear += THREADS256) {
        ...
        const int column = linear & 255;
        if (((row >> 4) & 1) == rank) {
            output[base + linear] = column <= row
                ? tile[local_index256(row, column)]
                : 0.0f;
        }
    }
}
...
void chol256_launch(torch::Tensor input, torch::Tensor output) {
    ...
    config.gridDim = dim3(batch * CLUSTER_BLOCKS256);
    config.blockDim = dim3(THREADS256);
    config.dynamicSmemBytes = SHARED_BYTES256;
    cudaLaunchAttribute attribute = {};
    attribute.id = cudaLaunchAttributeClusterDimension;
    attribute.val.clusterDim = dim3(CLUSTER_BLOCKS256, 1, 1);
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA256 = load_inline(
            name="chol256_exp043_v5_cluster2_b16_t512",
