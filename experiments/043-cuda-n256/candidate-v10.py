...
constexpr int TILE256 = 16;
constexpr int BK256 = 32;
constexpr int THREADS256 = 512;
constexpr int WARPS256 = THREADS256 / 32;
constexpr int TILE_STRIDE256 = 20;
constexpr int TILE_VALUES256 = TILE256 * TILE_STRIDE256;
constexpr int TILES_PER_DIM256 = N256 / TILE256;
...
void cholesky256_wmma16(const float* input, float* output) {
    ...
    for (int block = 0; block < N256; block += BK256) {
        ...
        for (int local = 0; local < BK256; ++local) {
            ...
            for (int linear = tid; linear < BK256 * BK256;
                 linear += THREADS256) {
                const int row = block + (linear >> 5);
                const int column = block + (linear & 31);
                if (row > pivot && column > pivot && column <= row) {
        ...
        if (row < N256) {
            ...
            for (int local = 0; local < BK256; ++local) {
                const int column = block + local;
                float value = tile[tile_index256(row, column)];
                #pragma unroll
                for (int prior = 0; prior < local; ++prior) {
                    value = fmaf(
                        -tile[tile_index256(row, block + prior)],
                        tile[tile_index256(column, block + prior)],
                        value);
                }
                tile[tile_index256(row, column)] = value * inverse_diag[local];
            }
        }
        __syncthreads();
        ...
        for (int tile_row = first_tile; tile_row < TILES_PER_DIM256; ++tile_row) {
            for (int tile_column = first_tile;
                ...
                #pragma unroll
                for (int panel_part = 0; panel_part < BK256;
                     panel_part += TILE256) {
                    const float* a_ptr = tile + tile_index256(
                        tile_row << 4, block + panel_part);
                    const float* b_ptr = tile + tile_index256(
                        tile_column << 4, block + panel_part);
                    #pragma unroll
                    for (int k = 0; k < TILE256; k += 8) {
                        wmma::fragment<
    ...
    for (int linear = tid; linear < N256 * N256; linear += THREADS256) {
        ...
        if (column > row) output[base + linear] = 0.0f;
    }
    int output_tile = 0;
    #pragma unroll 1
    for (int tile_row = 0; tile_row < TILES_PER_DIM256; ++tile_row) {
        for (int tile_column = 0; tile_column <= tile_row;
            ...
            if (tid < TILE256 * TILE256) {
                const int local_row = tid >> 4;
                const int local_column = tid & 15;
                if (tile_row != tile_column || local_column <= local_row) {
                    const int row = (tile_row << 4) + local_row;
                    const int column = (tile_column << 4) + local_column;
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA256 = load_inline(
            name="chol256_exp043_v10_wmma_tf32_b32_s20",
