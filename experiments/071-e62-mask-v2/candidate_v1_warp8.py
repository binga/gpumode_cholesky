...
// Experiment 071 variant 1: eight rows per 256-thread block. Each warp owns one
// row, giving coalesced 32-float stores while cutting the block count 8x versus
// exp069's one-block-per-row mask. Lower-triangular values remain untouched.
__global__ void e62_zero_upper_kernel(float* __restrict__ A, int n)
{
    const int warp = (int)threadIdx.x >> 5;
    const int lane = (int)threadIdx.x & 31;
    const int row_groups = (n + 7) >> 3;
    const long long group = (long long)blockIdx.x;
    const int b = (int)(group / (long long)row_groups);
    const int r = (int)(group % (long long)row_groups) * 8 + warp;
    if (r >= n) return;
...
void e62_zero_upper_launch(torch::Tensor A)
{
    ...
    const long long blocks =
        (long long)batch * (long long)((n + 7) >> 3);
    if (blocks <= 0) return;
