...
// Experiment 071 variant 3: 64x64 output tiles. A 256-thread CTA writes one
// strict-upper tile, with diagonal-tile predicates and an immediate return for
// tiles wholly below the diagonal. This changes both work decomposition and
// launch geometry from the row-owned exp069/v1/v2 masks.
__global__ void e62_zero_upper_kernel(float* __restrict__ A, int n)
{
    ...
    for (int idx = (int)threadIdx.x; idx < 64 * 64; idx += (int)blockDim.x) {
        const int r = r0 + (idx >> 6);
        const int c = c0 + (idx & 63);
        if (r < n && c < n && c > r) {
            A[((long long)b * (long long)n + r) * (long long)n + c] = 0.0f;
        }
    }
}
...
void e62_zero_upper_launch(torch::Tensor A)
{
    ...
    const unsigned int tiles = (unsigned int)((n + 63) >> 6);
    e62_zero_upper_kernel<<<dim3(tiles, tiles, (unsigned int)batch), 256>>>(
