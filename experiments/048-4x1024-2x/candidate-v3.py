"""Experiment 048 V3: cooperative 4x1024 with reciprocal panel solves."""
        ...
        for (int bi = kt + 1 + local_block; bi < NT; bi += GROUP_BLOCKS) {
            ...
            if (warp == 0) {
                ...
                for (int column = 0; column < 32; ++column) {
                    ...
                    // The diagonal factor is fixed during this phase.  Use the
                    // hardware reciprocal-multiply path instead of 32 chained
                    // FP32 divisions per row; the ranked split32 path already
                    // validates reciprocal diagonal solves at this shape.
...
if torch.cuda.is_available():
    try:
        ...
        _COOP1024 = load_inline(
            name="chol1024_exp048_v3_recip_panel",
