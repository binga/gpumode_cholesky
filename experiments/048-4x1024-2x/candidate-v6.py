"""Experiment 048 V6: cluster Cholesky with dual-warp panel ownership."""
    ...
    for (int kt = 0; kt < NT; ++kt) {
        ...
        if (local_block == 0 && warp == 0) {
            ...
            for (int item = 0; item < 32; ++item) {
                ...
                tile[0][linear >> 5][linear & 31] =
                    matrix[(size_t)(k0 + (linear >> 5)) * N + k0 + (linear & 31)];
            }
            __syncwarp();
            #pragma unroll
            for (int column = 0; column < 32; ++column) {
                row_values[column] = tile[0][lane][column];
            }
            #pragma unroll
        ...
        // Phase 2: each CTA owns at most two panel tiles.  Warps 0 and 1
        // stage/solve them concurrently, removing the serial second-tile pass
        // introduced by the 16-CTA cluster while retaining coalesced staging.
        if (warp < 2) {
            const int bi = kt + 1 + local_block + warp * GROUP_BLOCKS;
            if (bi < NT) {
            ...
            __syncwarp();
                ...
                #pragma unroll
                for (int column = 0; column < 32; ++column) {
                    values[column] = tile[warp][lane][column];
                }
                #pragma unroll
                for (int column = 0; column < 32; ++column) {
                ...
                #pragma unroll
                for (int column = 0; column < 32; ++column) {
                    tile[warp][lane][column] = values[column];
                }
            __syncwarp();
            #pragma unroll
            for (int item = 0; item < 32; ++item) {
...
if torch.cuda.is_available():
    try:
        ...
        _COOP1024 = load_inline(
            name="chol1024_exp048_v6_cluster16_dualpanel",
