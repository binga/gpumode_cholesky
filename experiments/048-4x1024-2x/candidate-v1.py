...
if _HAVE_TRITON:
    ...
    def _resident_panel128(
        ...
        """Solve one complete 128-wide block column with one global read/write.

        The four diagonal 32x32 inverses are stored as
        ``[substep, batch, 32, 32]``.  Four row fragments remain live while the
        blocked triangular solve advances through the diagonal superpanel, so
        the rows below the diagonal are not reread by seven separate kernels.
        This is the graph-capturable experiment-048 V1 architecture.
        """
        rt = tl.program_id(0)
        b = tl.program_id(1).to(tl.int64)
        rows = rt * TILE_R + tl.arange(0, TILE_R)
        ...
        row_index = j + 128 + rows

        off0 = base + row_index[:, None] * n + (j + c)[None, :]
        off1 = base + row_index[:, None] * n + (j + 32 + c)[None, :]
        off2 = base + row_index[:, None] * n + (j + 64 + c)[None, :]
        off3 = base + row_index[:, None] * n + (j + 96 + c)[None, :]
        if FIRST:
            p0 = tl.load(src_ptr + off0, mask=valid[:, None], other=0.0)
            p1 = tl.load(src_ptr + off1, mask=valid[:, None], other=0.0)
            p2 = tl.load(src_ptr + off2, mask=valid[:, None], other=0.0)
            p3 = tl.load(src_ptr + off3, mask=valid[:, None], other=0.0)
        else:
            p0 = tl.load(out_ptr + off0, mask=valid[:, None], other=0.0)
            p1 = tl.load(out_ptr + off1, mask=valid[:, None], other=0.0)
            p2 = tl.load(out_ptr + off2, mask=valid[:, None], other=0.0)
            p3 = tl.load(out_ptr + off3, mask=valid[:, None], other=0.0)

        inv_base = b * 1024 + c[:, None] * 32 + c[None, :]
        inv0 = tl.load(inv_ptr + inv_base)
        inv1 = tl.load(inv_ptr + 4 * 1024 + inv_base)
        inv2 = tl.load(inv_ptr + 8 * 1024 + inv_base)
        inv3 = tl.load(inv_ptr + 12 * 1024 + inv_base)

        x0 = tl.dot(p0, tl.trans(inv0), input_precision=PREC,
                    out_dtype=tl.float32)
        l10 = tl.load(out_ptr + base + (j + 32 + c)[:, None] * n +
                      (j + c)[None, :])
        l20 = tl.load(out_ptr + base + (j + 64 + c)[:, None] * n +
                      (j + c)[None, :])
        l30 = tl.load(out_ptr + base + (j + 96 + c)[:, None] * n +
                      (j + c)[None, :])
        p1 -= tl.dot(x0, tl.trans(l10), input_precision=PREC,
                     out_dtype=tl.float32)
        p2 -= tl.dot(x0, tl.trans(l20), input_precision=PREC,
                     out_dtype=tl.float32)
        p3 -= tl.dot(x0, tl.trans(l30), input_precision=PREC,
                     out_dtype=tl.float32)

        x1 = tl.dot(p1, tl.trans(inv1), input_precision=PREC,
                    out_dtype=tl.float32)
        l21 = tl.load(out_ptr + base + (j + 64 + c)[:, None] * n +
                      (j + 32 + c)[None, :])
        l31 = tl.load(out_ptr + base + (j + 96 + c)[:, None] * n +
                      (j + 32 + c)[None, :])
        p2 -= tl.dot(x1, tl.trans(l21), input_precision=PREC,
                     out_dtype=tl.float32)
        p3 -= tl.dot(x1, tl.trans(l31), input_precision=PREC,
                     out_dtype=tl.float32)

        x2 = tl.dot(p2, tl.trans(inv2), input_precision=PREC,
                    out_dtype=tl.float32)
        l32 = tl.load(out_ptr + base + (j + 96 + c)[:, None] * n +
                      (j + 64 + c)[None, :])
        p3 -= tl.dot(x2, tl.trans(l32), input_precision=PREC,
                     out_dtype=tl.float32)
        x3 = tl.dot(p3, tl.trans(inv3), input_precision=PREC,
                    out_dtype=tl.float32)

        o0 = out_ptr + base + row_index[:, None] * n + (j + c)[None, :]
        o1 = out_ptr + base + row_index[:, None] * n + (j + 32 + c)[None, :]
        o2 = out_ptr + base + row_index[:, None] * n + (j + 64 + c)[None, :]
        o3 = out_ptr + base + row_index[:, None] * n + (j + 96 + c)[None, :]
        tl.store(o0, x0, mask=valid[:, None])
        tl.store(o1, x1, mask=valid[:, None])
        tl.store(o2, x2, mask=valid[:, None])
        tl.store(o3, x3, mask=valid[:, None])

        # The factor contract requires an explicitly zero strict upper half.
        z = tl.zeros((32, TILE_R), tl.float32)
        tl.store(out_ptr + base + (j + c)[:, None] * n + row_index[None, :],
                 z, mask=valid[None, :])
        tl.store(out_ptr + base + (j + 32 + c)[:, None] * n + row_index[None, :],
                 z, mask=valid[None, :])
        tl.store(out_ptr + base + (j + 64 + c)[:, None] * n + row_index[None, :],
                 z, mask=valid[None, :])
        tl.store(out_ptr + base + (j + 96 + c)[:, None] * n + row_index[None, :],
                 z, mask=valid[None, :])
    ...
    def _split32_launch(
        ...
        resident_panel = batch == 4 and n == 1024
        if resident_panel:
            _RESIDENT_1024_HITS += 1
        tile = _SPLIT32_TILE
        ft = src is not None
        if not ft:
            src = work
        ...
        for nb in _nb_schedule(batch, n):
            ...
            for k in range(j, panel_end, 32):
                step_inv = dinv[(k - j) // 32] if resident_panel else dinv
                if cuda_micro:
                    _MICRO32_HITS += 1
                    _MICRO32.micro32_launch(
                        src, work, step_inv, n, k, 1 if (ft and k == 0) else 0
                    )
                else:
                ...
                remaining = (
                    panel_end - k - 32 if resident_panel else n - k - 32
                )
                if remaining <= 0:
                    break
            ...
            rem_out = n - panel_end
            if resident_panel and rem_out > 0:
                _resident_panel128[(triton.cdiv(rem_out, 64), batch)](
                    work,
                    dinv,
                    src,
                    n=n,
    ...
    def _split32_factor(data: torch.Tensor) -> torch.Tensor:
        ...
        if mode == "eager":
            ...
            if dinv is None:
                inv_shape = ((4, batch, 32, 32) if (batch, n) == (4, 1024)
                             else (batch, 32, 32))
                dinv = torch.empty(*inv_shape, device=data.device,
                                   dtype=torch.float32)
...
_RESIDENT_1024_HITS = 0
_RESIDENT_1024_FALLBACKS = 0
_RESIDENT_1024_ERROR = None
