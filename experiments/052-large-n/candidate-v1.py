...
#
# Measured against the ranked incumbent (results/inc-890798-shapediag.json) the
# shipped `_left_looking_large` spends, per shape:
#
...
#
# The triangular work and the scaffolding around it, not the O(n^3) GEMM, is
# the cost.  Three changes:
#
#   1. `_blocked_tri_inv` replaces `_tri_inv_recursive`.  The shipped recursion
#      bottoms out in one dense-identity `solve_triangular` per leaf (80us for
#      a 512x512 leaf = 1.7 TFLOP/s) and emits ~4 elementwise kernels per
#      internal node.  The blocked form inverts every leaf in ONE batched
#      triangular solve and combines the levels with strided batched GEMMs on
#      views of the factor, so a whole nb x nb inverse costs 1 trsm +
#      2*log2(nb/base) bmms and log2(nb/base) copies.
#   2. The diagonal-block update and the below-diagonal panel update become a
#      single GEMM over the full block column (tf32 shapes only; 32768 keeps
#      the shipped split so its MXFP8 panel product and its FP32-accumulated
#      diagonal update are numerically unchanged).
#   3. 1x8192 moves onto the explicit-inverse panel apply, replacing cuBLAS
#      `trsm_right_kernel` with a tensor-core GEMM.
#
# No new cuSOLVER call site: the per-block `cholesky_ex` on the nb x nb
# diagonal is exactly the one the ranked path already issues, at the same
# block sizes and the same call count.
# ---------------------------------------------------------------------------
...
def _blocked_tri_inv(lower: torch.Tensor, base: int = 256) -> torch.Tensor:
    ...
    Level 0 inverts the `base`-sized diagonal blocks -- which are mutually
    independent -- with a single batched `solve_triangular`.  Each later level
    doubles the block size using the 2x2 identity
    inv([[A,0],[B,C]]) = [[Ai,0],[-Ci B Ai, Ci]], evaluated for every pair at
    that level at once as two strided batched GEMMs straight off views of
    `lower` / `inverse`.  Depth is log2(n/base) launches instead of n/base
    serialised triangular solves.
    """
    n = lower.shape[0]
...
def _large_v2(mat: torch.Tensor, nb: int, update: str, inv_base: int) -> torch.Tensor:
    ...
    try:
        ...
        for k in range(0, n, nb):
            ...
            j = k + kb
            if update == "mxfp8":
                # Shipped 32768 schedule: FP32-accumulated TF32 diagonal
                # update, MXFP8 panel update, unchanged.
                ...
                # One GEMM for the diagonal block and the panel below it.
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if is_f32_cuda and batch == 1 and n in _LARGE_CFG_V2:
        try:
