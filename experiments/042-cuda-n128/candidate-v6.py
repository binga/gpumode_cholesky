"""Experiment 042 V6: dual-accumulator blocked-16 CUDA Cholesky."""
            ...
            for (int column = block_end + lane;
                ...
                for (int k = 0; k < BK; k += 2) {
                    update0 = fmaf(
                        ...
                        update0);
                    update1 = fmaf(
                        tile[trailing_row * TILE_STRIDE + block + k + 1],
                        tile[column * TILE_STRIDE + block + k + 1],
                        update1);
                }
                tile[trailing_row * TILE_STRIDE + column] -= update0 + update1;
            }
        }
        __syncthreads();
    }
