"""Experiment 042 V1 device-clock phase instrumentation."""
...
__global__ void cholesky128_rank2(
        const float* input,
        float* output,
    ...
    unsigned long long mark0 = 0;
    unsigned long long mark1 = 0;
    unsigned long long mark2 = 0;
    if (row == 0) mark0 = global_nanoseconds();
    __syncthreads();
    ...
    __syncthreads();
    if (row == 0) mark1 = global_nanoseconds();
    __syncthreads();
    ...
    if (row == 0) mark2 = global_nanoseconds();
    __syncthreads();
    ...
    __syncthreads();
    if (row == 0) {
        const unsigned long long mark3 = global_nanoseconds();
        timings[(size_t)blockIdx.x * 4 + 0] = mark0;
        timings[(size_t)blockIdx.x * 4 + 1] = mark1;
        timings[(size_t)blockIdx.x * 4 + 2] = mark2;
        timings[(size_t)blockIdx.x * 4 + 3] = mark3;
    }
}

__global__ void barrier128_floor(
        unsigned long long* timings,
        int* sink) {
    const int row = threadIdx.x;
    __shared__ volatile int baton;
    unsigned long long mark0 = 0;
    if (row == 0) mark0 = global_nanoseconds();
    __syncthreads();
    int value = 0;
    ...
    for (int iteration = 0; iteration < 128; ++iteration) {
        if (row == (iteration & 127)) baton = iteration;
        __syncthreads();
        value += baton;
        __syncthreads();
    }
    if (row == 0) {
        const unsigned long long mark1 = global_nanoseconds();
        timings[(size_t)blockIdx.x * 2 + 0] = mark0;
        timings[(size_t)blockIdx.x * 2 + 1] = mark1;
...
void barrier128_launch(torch::Tensor timings, torch::Tensor sink) {
    barrier128_floor<<<dim3(256), dim3(N)>>>(
        reinterpret_cast<unsigned long long*>(timings.data_ptr<int64_t>()),
        sink.data_ptr<int>());
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA128 = load_inline(
            name="chol128_exp042_v1_phase",
            cpp_sources=(
                "void chol128_launch(torch::Tensor, torch::Tensor, torch::Tensor);"
                "void barrier128_launch(torch::Tensor, torch::Tensor);"
            ),
            cuda_sources=_CUDA128_SOURCE,
            functions=["chol128_launch", "barrier128_launch"],
...
def custom_kernel(data):
    ...
    if (
        ...
        timings = torch.empty((256, 4), dtype=torch.int64, device=data.device)
...
def barrier_probe():
    timings = torch.empty((256, 2), dtype=torch.int64, device="cuda")
    sink = torch.empty(256, dtype=torch.int32, device="cuda")
    _CUDA128.barrier128_launch(timings, sink)
    return timings, sink
