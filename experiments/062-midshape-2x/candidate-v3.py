...
#define E62_SMEM_F  (E62_P_OFF + 32 * E62_PLD)
...
__device__ __forceinline__ void e62_tri_inv32(const float* __restrict__ Sb,
                                              float* __restrict__ Qi, int lane)
{
    const float rdiag = __frcp_rn(Sb[lane * E62_LD + lane]);
    float x[32];
    #pragma unroll
    for (int i = 0; i < 32; ++i) {
        float s0 = (i == lane) ? 1.0f : 0.0f;
        float s1 = 0.0f, s2 = 0.0f, s3 = 0.0f;
        #pragma unroll
        for (int p = 0; p < 32; p += 4) {
            if (p + 0 < i) s0 -= Sb[i * E62_LD + p + 0] * x[p + 0];
            if (p + 1 < i) s1 -= Sb[i * E62_LD + p + 1] * x[p + 1];
            if (p + 2 < i) s2 -= Sb[i * E62_LD + p + 2] * x[p + 2];
            if (p + 3 < i) s3 -= Sb[i * E62_LD + p + 3] * x[p + 3];
        }
        const float ri = __shfl_sync(0xffffffffu, rdiag, i);
        x[i] = (i >= lane) ? ((s0 + s1) + (s2 + s3)) * ri : 0.0f;
    }
    #pragma unroll
