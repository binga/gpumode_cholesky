...
# Only the eager-mode split32 shapes are enrolled. The kernel launches on the
# default stream (popcorn's source scan rejects any stream API, including
# `getCurrentCUDAStream`), which is correct in eager mode but cannot be
# captured into the CUDA graphs the other split32 shapes replay -- measured
# 0.38-0.52x there through the finiteness fallback.
_MICRO32_SHAPES = {
    (640, 512),
}
