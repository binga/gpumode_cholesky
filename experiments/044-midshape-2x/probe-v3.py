...
# ---------------------------------------------------------------------------
# Experiment 044 design probe: time competing diagonal-micro architectures in
# one Modal run. Every variant factors the 32x32 block at (k, k) of a
# (batch, n, n) work tensor and publishes its triangular inverse, exactly like
# `_micro_potrf_gj32`, so their per-launch costs are directly comparable.
# ---------------------------------------------------------------------------
_MIDPROBE_SOURCE = r"""
#include <torch/extension.h>
#include <cuda_runtime.h>
...
// --- variant "null": launch-and-touch floor -------------------------------
__global__ void probe_null(float* work, int batch, int n, int k) {
    if (threadIdx.x == 0 && blockIdx.x == 0) work[0] = work[0];
}

// --- variant "coal": shared coalesced staging, rank-1, uncapped registers --
__global__ __launch_bounds__(THREADS)
void probe_coal(const float* __restrict__ src, float* __restrict__ work,
    ...
    __shared__ float pivot_column[WARPS][BK];
    ...
    #pragma unroll
    for (int p = 0; p < BK; ++p) {
        const float pivot = __shfl_sync(FULL, values[p], p);
        const float reciprocal = rsqrtf(pivot);
        if (lane == p) reciprocal_row = reciprocal;
        const float scale = (lane >= p) ? values[p] * reciprocal : 0.0f;
        values[p] = scale;
        ...
        for (int c = 0; c < BK; ++c) {
            if (c > p) {
                values[c] = (c <= lane)
                    ? fmaf(-scale, column[c], values[c]) : values[c];
            }
        }
        __syncwarp();
    }
...
// --- variant "rank2": two pivots per warp rendezvous ----------------------
__global__ __launch_bounds__(THREADS)
void probe_rank2(const float* __restrict__ src, float* __restrict__ work,
                 float* __restrict__ inv, int batch, int n, int k, int first) {
    const int warp = threadIdx.x >> 5;
    const int lane = threadIdx.x & 31;
    const int matrix = blockIdx.x * WARPS + warp;
    ...
    __shared__ float staging[WARPS][BK][BK + 1];
    __shared__ float pivot0_s[WARPS][BK];
    __shared__ float pivot1_s[WARPS][BK];
    float (*tile)[BK + 1] = staging[warp];
    float* pivot0 = pivot0_s[warp];
    float* pivot1 = pivot1_s[warp];
    ...
    float reciprocal_row = 0.0f;
    #pragma unroll
    for (int iteration = 0; iteration < BK / 2; ++iteration) {
        const int p = 2 * iteration;
        const int q = p + 1;
        const float app = __shfl_sync(FULL, values[p], p);
        const float rec0 = rsqrtf(app);
        const float scale0 = (lane >= p) ? values[p] * rec0 : 0.0f;
        values[p] = scale0;
        pivot0[lane] = scale0;
        const float lqp = __shfl_sync(FULL, scale0, q);
        float vq = (lane >= q) ? fmaf(-scale0, lqp, values[q]) : values[q];
        const float aqq = __shfl_sync(FULL, vq, q);
        const float rec1 = rsqrtf(aqq);
        const float scale1 = (lane >= q) ? vq * rec1 : 0.0f;
        values[q] = scale1;
        pivot1[lane] = scale1;
        if (lane == p) reciprocal_row = rec0;
        if (lane == q) reciprocal_row = rec1;
        __syncwarp();
        #pragma unroll
        for (int c = 0; c < BK; ++c) {
            if (c > q) {
                ...
                if (c <= lane) {
                    value = fmaf(-scale0, pivot0[c], value);
                    value = fmaf(-scale1, pivot1[c], value);
                }
                values[c] = value;
            }
        }
    ...
    for (int r = 0; r < BK; ++r) {
        ...
        inverse[r] = (r >= lane) ? accumulator * pivot0[r] : 0.0f;
    }
    float* inverse_out = inv + (size_t)matrix * BK * BK + lane;
    #pragma unroll
    for (int r = 0; r < BK; ++r) inverse_out[r * BK] = inverse[r];
...
// --- variant "rank4": four pivots per warp rendezvous ---------------------
__global__ __launch_bounds__(THREADS)
void probe_rank4(const float* __restrict__ src, float* __restrict__ work,
                 float* __restrict__ inv, int batch, int n, int k, int first) {
    const int warp = threadIdx.x >> 5;
    const int lane = threadIdx.x & 31;
    const int matrix = blockIdx.x * WARPS + warp;
...
// --- variant "noinv": rank-1 coalesced, inverse omitted -------------------
__global__ __launch_bounds__(THREADS)
void probe_noinv(const float* __restrict__ src, float* __restrict__ work,
                 float* __restrict__ inv, int batch, int n, int k, int first) {
    const int warp = threadIdx.x >> 5;
    const int lane = threadIdx.x & 31;
    const int matrix = blockIdx.x * WARPS + warp;
...
void NAME(torch::Tensor src, torch::Tensor work, torch::Tensor inv,         \
          int64_t n, int64_t k, int64_t first) {                            \
    const int batch = (int)work.size(0);                                    \
    const int blocks = (batch + WARPS - 1) / WARPS;                         \
    KERNEL<<<dim3(blocks), dim3(THREADS), 0,                                \
             at::cuda::getCurrentCUDAStream()>>>(                           \
...
LAUNCHER(probe_coal_launch, probe_coal)
LAUNCHER(probe_rank2_launch, probe_rank2)
LAUNCHER(probe_rank4_launch, probe_rank4)
LAUNCHER(probe_noinv_launch, probe_noinv)

void probe_null_launch(torch::Tensor src, torch::Tensor work,
                       torch::Tensor inv, int64_t n, int64_t k,
                       int64_t first) {
    const int batch = (int)work.size(0);
    const int blocks = (batch + WARPS - 1) / WARPS;
    probe_null<<<dim3(blocks), dim3(THREADS), 0,
                 at::cuda::getCurrentCUDAStream()>>>(
        work.data_ptr<float>(), batch, (int)n, (int)k);
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
}
"""
...
_MIDPROBE = None
_MIDPROBE_ERROR = None
if torch.cuda.is_available():
    try:
        from torch.utils.cpp_extension import load_inline

        _MIDPROBE = load_inline(
            name="midprobe_exp044_v2",
            cpp_sources=(
                "void probe_coal_launch(torch::Tensor, torch::Tensor, "
                "torch::Tensor, int64_t, int64_t, int64_t);\n"
                "void probe_rank2_launch(torch::Tensor, torch::Tensor, "
                "torch::Tensor, int64_t, int64_t, int64_t);\n"
                "void probe_rank4_launch(torch::Tensor, torch::Tensor, "
                "torch::Tensor, int64_t, int64_t, int64_t);\n"
                "void probe_noinv_launch(torch::Tensor, torch::Tensor, "
                "torch::Tensor, int64_t, int64_t, int64_t);\n"
                "void probe_null_launch(torch::Tensor, torch::Tensor, "
                "torch::Tensor, int64_t, int64_t, int64_t);"
            ),
            cuda_sources=_MIDPROBE_SOURCE,
            functions=[
                "probe_coal_launch",
                "probe_rank2_launch",
                "probe_rank4_launch",
        ...
        _MIDPROBE_ERROR = repr(exc)


# ---------------------------------------------------------------------------
# Experiment 044 probe round 2: fused whole-block diagonal factorization.
# One CTA per matrix factors an NB x NB diagonal block of a stride-n matrix
# entirely in shared memory, replacing NB/32 `_micro_potrf_gj32` launches and
# the `_panel_inner32*` launches between them.
# ---------------------------------------------------------------------------
_BLOCKPROBE_SOURCE = r"""
#include <torch/extension.h>
#include <cuda_runtime.h>
#include <c10/cuda/CUDAStream.h>

template <int NB, int BKI, int THREADS>
__global__ __launch_bounds__(THREADS)
void block_factor(float* __restrict__ work, int n, int j) {
    const int tid = threadIdx.x;
    const int warp = tid >> 5;
    const int lane = tid & 31;
    constexpr int WARPS = THREADS >> 5;
    constexpr int STRIDE = NB + 1;
    extern __shared__ float tile[];
    __shared__ float reciprocal;
    __shared__ float inverse_diag[BKI];

    float* block = work + (size_t)blockIdx.x * n * n + (size_t)j * n + j;

    for (int linear = tid; linear < NB * NB; linear += THREADS) {
        const int row = linear / NB;
        const int column = linear - row * NB;
        tile[row * STRIDE + column] = block[(size_t)row * n + column];
    }
    __syncthreads();

    #pragma unroll 1
    for (int start = 0; start < NB; start += BKI) {
        const int block_end = start + BKI;

        #pragma unroll
        for (int local = 0; local < BKI; ++local) {
            const int pivot = start + local;
            if (tid == 0) {
                reciprocal = rsqrtf(tile[pivot * STRIDE + pivot]);
                inverse_diag[local] = reciprocal;
                tile[pivot * STRIDE + pivot] *= reciprocal;
            }
            __syncthreads();
            const int panel_row = pivot + 1 + tid;
            if (panel_row < block_end) {
                tile[panel_row * STRIDE + pivot] *= reciprocal;
            }
            __syncthreads();
            for (int linear = tid; linear < BKI * BKI; linear += THREADS) {
                const int row = start + (linear / BKI);
                const int column = start + (linear % BKI);
                if (row > pivot && column > pivot && column <= row) {
                    const int offset = row * STRIDE + column;
                    tile[offset] = fmaf(
                        -tile[row * STRIDE + pivot],
                        tile[column * STRIDE + pivot],
                        tile[offset]);
                }
            }
            __syncthreads();
        ...
        for (int row = block_end + tid; row < NB; row += THREADS) {
            #pragma unroll
            for (int local = 0; local < BKI; ++local) {
                const int column = start + local;
                float value = tile[row * STRIDE + column];
                #pragma unroll
                for (int prior = 0; prior < BKI; ++prior) {
                    if (prior < local) {
                        value = fmaf(
                            -tile[row * STRIDE + start + prior],
                            tile[column * STRIDE + start + prior],
                            value);
                    }
                }
                tile[row * STRIDE + column] = value * inverse_diag[local];
            }
        }
        __syncthreads();
        ...
        for (int trailing_row = block_end + warp;
            ...
            for (int column = block_end + lane;
                ...
                for (int q = 0; q < BKI; ++q) {
                    update = fmaf(
                        tile[trailing_row * STRIDE + start + q],
                        tile[column * STRIDE + start + q],
                        update);
                }
                tile[trailing_row * STRIDE + column] -= update;
            }
        }
        __syncthreads();
    }
    ...
    for (int linear = tid; linear < NB * NB; linear += THREADS) {
        ...
        block[(size_t)row * n + column] =
            column <= row ? tile[row * STRIDE + column] : 0.0f;
    }
}

template <int NB, int BKI, int THREADS>
void run_block(torch::Tensor work, int64_t n, int64_t j) {
    constexpr int shared_bytes = NB * (NB + 1) * sizeof(float);
    auto kernel = block_factor<NB, BKI, THREADS>;
    static bool configured = false;
    cudaError_t attr = cudaSuccess;
    if (!configured) {
        attr = cudaFuncSetAttribute(
            kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shared_bytes);
        configured = true;
    }
    cudaFuncAttributes info;
    cudaFuncGetAttributes(&info, kernel);
    const int batch = (int)work.size(0);
    kernel<<<dim3(batch), dim3(THREADS), shared_bytes,
             at::cuda::getCurrentCUDAStream()>>>(
        work.data_ptr<float>(), (int)n, (int)j);
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess,
                "NB=", NB, " BKI=", BKI, " THREADS=", THREADS,
                " dyn=", shared_bytes, " setattr=", (int)attr,
                " maxThreads=", info.maxThreadsPerBlock,
                " staticShared=", (int)info.sharedSizeBytes,
                " regs=", info.numRegs, " localBytes=", (int)info.localSizeBytes,
                " -> ", cudaGetErrorString(status));
}

void block128_bk16(torch::Tensor w, int64_t n, int64_t j) {
    run_block<128, 16, 256>(w, n, j);
}
void block128_bk32(torch::Tensor w, int64_t n, int64_t j) {
    run_block<128, 32, 256>(w, n, j);
}
void block128_bk16_t512(torch::Tensor w, int64_t n, int64_t j) {
    run_block<128, 16, 512>(w, n, j);
}
void block64_bk16(torch::Tensor w, int64_t n, int64_t j) {
    run_block<64, 16, 256>(w, n, j);
}
void block256_bk16(torch::Tensor w, int64_t n, int64_t j) {
    run_block<256, 16, 512>(w, n, j);
}
"""

_BLOCKPROBE = None
_BLOCKPROBE_ERROR = None
if torch.cuda.is_available():
    try:
        from torch.utils.cpp_extension import load_inline

        _BLOCKPROBE = load_inline(
            name="blockprobe_exp044_v3",
            cpp_sources=(
                "void block128_bk16(torch::Tensor, int64_t, int64_t);\n"
                "void block128_bk32(torch::Tensor, int64_t, int64_t);\n"
                "void block128_bk16_t512(torch::Tensor, int64_t, int64_t);\n"
                "void block64_bk16(torch::Tensor, int64_t, int64_t);\n"
                "void block256_bk16(torch::Tensor, int64_t, int64_t);"
            ),
            cuda_sources=_BLOCKPROBE_SOURCE,
            functions=[
                "block128_bk16",
                "block128_bk32",
                "block128_bk16_t512",
        ...
        _BLOCKPROBE_ERROR = repr(exc)


def block_probe():
    """Cost of one fused NB-wide diagonal block factorization, per launch."""
    rows = []
    if _BLOCKPROBE_ERROR is not None:
        return [{"name": "block_load_error", "us": 0.0, "ok": False,
                 "error": _BLOCKPROBE_ERROR}]

    for batch, n in ((16, 512), (4, 1024), (2, 2048)):
        torch.manual_seed(0)
        base = torch.randn(batch, n, n, device="cuda", dtype=torch.float32)
        spd = (base @ base.transpose(-1, -2) / n) + torch.eye(
            n, device="cuda") * n
        reference = torch.linalg.cholesky(spd.double()).float()
        work = torch.empty_like(spd)

        for label, fn, nb in (
            ("block64-bk16-t256", _BLOCKPROBE.block64_bk16, 64),
            ("block128-bk16-t256", _BLOCKPROBE.block128_bk16, 128),
            ("block128-bk32-t256", _BLOCKPROBE.block128_bk32, 128),
            ("block128-bk16-t512", _BLOCKPROBE.block128_bk16_t512, 128),
        ):
            work.copy_(spd)
            try:
                fn(work, n, 0)
                ...
                rows.append({"name": f"{label} batch={batch} n={n}",
                             "us": 0.0, "ok": False, "error": repr(exc)[:400]})
                return rows
            err = float(
                (work[:, :nb, :nb].tril()
                 - reference[:, :nb, :nb]).abs().max()
            )
            for _ in range(3):
                fn(work, n, 0)
            torch.cuda.synchronize()
            ...
            rows.append({
                "name": f"{label} batch={batch} n={n}",
                "us": round(single_us, 3),
                "blocks_for_n": n // nb,
                "chain_us": round(single_us * (n // nb), 1),
                "max_abs_err": err,
                "ok": True,
            })
        del base, spd, reference, work
        torch.cuda.empty_cache()
    return rows


def mid_probe():
    """Time one diagonal-micro launch chain per variant at 16x512."""
    import math
    ...
    def time_chain(launch, label, check=False):
        # accuracy: factor only the (0,0) block from a fresh copy
        residual = float("nan")
        if check:
            work.copy_(spd)
            launch(spd, work, dinv, n, 0, 0)
            torch.cuda.synchronize()
            got = work[:, :32, :32].tril()
            want = reference[:, :32, :32]
            residual = float((got - want).abs().max())
            eye = torch.eye(32, device="cuda").expand(batch, 32, 32)
            inv_err = float(
                (dinv.view(batch, 32, 32) @ want - eye).abs().max()
            )
        else:
            inv_err = float("nan")
        work.copy_(spd)
        ...
        rows.append(
            {
                "name": label,
                "us": round(chain_us, 3),
                "per_call_us": round(chain_us / steps, 3),
                "launches": steps,
                "max_abs_err": None if math.isnan(residual) else residual,
                "inv_err": None if math.isnan(inv_err) else inv_err,
                "ok": True,
            }
        )

    if _MIDPROBE_ERROR is not None:
        return [{"name": "load_error", "us": 0.0, "ok": False,
                 "error": _MIDPROBE_ERROR}]

    time_chain(_MIDPROBE.probe_null_launch, "null-launch-floor")
    time_chain(_MIDPROBE.probe_noinv_launch, "coalesced-rank1-noinverse")
    time_chain(_MIDPROBE.probe_coal_launch, "coalesced-rank1", check=True)
    time_chain(_MIDPROBE.probe_rank2_launch, "coalesced-rank2", check=True)
    time_chain(_MIDPROBE.probe_rank4_launch, "coalesced-rank4", check=True)
    time_chain(
        lambda s, w, i, nn, kk, f: _MICRO32.micro32_launch(s, w, i, nn, kk, f),
        "v1-uncoalesced-rank1",
        check=True,
    )
    return rows


_ORIGINAL_MID_PROBE = mid_probe


def mid_probe():
    return _ORIGINAL_MID_PROBE() + block_probe()
