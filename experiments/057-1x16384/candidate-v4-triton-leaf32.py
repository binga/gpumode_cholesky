"""Experiment 057 V4: Triton base-32 leaves for the trsm-free inverse.
...
try:
    ...
    _EXP057_V4_HAVE_TRITON = True
except Exception:
    triton = None
    tl = None
    _EXP057_V4_HAVE_TRITON = False


_EXP057_V4_ATTEMPTS = 0
_EXP057_V4_HITS = 0
_EXP057_V4_FALLBACKS = 0
_EXP057_V4_ERROR = None
_EXP057_V4_INVERSE_CALLS = 0
_EXP057_V4_TRITON_LEAF_HITS = 0


if _EXP057_V4_HAVE_TRITON:
...
def _triton_leaf32_inverse(lower: torch.Tensor) -> torch.Tensor:
    """Invert 32x32 leaves in one custom launch, then combine by GEMM."""
    global _EXP057_V4_INVERSE_CALLS, _EXP057_V4_TRITON_LEAF_HITS
    _EXP057_V4_INVERSE_CALLS += 1
    n = lower.shape[0]
    if not _EXP057_V4_HAVE_TRITON or n % 32 or (n & (n - 1)):
...
def _factor_1x16384(mat: torch.Tensor) -> torch.Tensor:
    ...
    try:
        ...
        for k in range(0, n, nb):
            ...
            if j >= n:
                break
            inverse = _triton_leaf32_inverse(lkk)
...
def custom_kernel(data: torch.Tensor) -> torch.Tensor:
    global _EXP057_V4_ATTEMPTS, _EXP057_V4_HITS, _EXP057_V4_FALLBACKS
    global _EXP057_V4_ERROR
    target = (
        data.is_cuda
    ...
    try:
        ...
        _EXP057_V4_ERROR = "missing Triton leaf hit or non-finite diagonal"
    except Exception as exc:
        _EXP057_V4_ERROR = repr(exc)
    _EXP057_V4_FALLBACKS += 1
    return _ranked.custom_kernel(data)
