...
# ---------------------------------------------------------------------------
# Experiment 058 V5: in-place blocked inverse plus FP16 panel application.
#
# The ranked recursive inverse bottoms out in 512x512 identity solves and
# serializes 112 cuBLAS TRSM launches over the eight 4096-wide panels.  Invert
# all independent 256x256 diagonal leaves in one batched solve, then combine
# each tree level with batched GEMMs. Apply the panel through that inverse with
# FP16 inputs and FP32 accumulation/output: FP16 retains TF32's 10-bit
# significand while its wider exponent avoids V3's E4M3 failure. Every other
# arithmetic path and dispatch remains unchanged. At each inverse tree level,
# write the negative off-diagonal product directly into its final strided view
# with baddbmm(beta=0, alpha=-1), removing an outer temporary, negation, and
# copy.
# ---------------------------------------------------------------------------
_BLOCKED_INV_32768_HITS = 0
_FP16_SOLVE_32768_HITS = 0
_INPLACE_INV_COMBINE_32768_HITS = 0


def _blocked_tri_inv_32768(
    lower: torch.Tensor, base: int = 256
) -> torch.Tensor:
    global _INPLACE_INV_COMBINE_32768_HITS
    ...
    while size < n:
        ...
        _INPLACE_INV_COMBINE_32768_HITS += 1
        size = step
    return inverse


def _fp16_solve_32768(
    panel: torch.Tensor, inverse: torch.Tensor
) -> torch.Tensor:
    global _FP16_SOLVE_32768_HITS
    ...
    _FP16_SOLVE_32768_HITS += 1
    return solved


def _left_looking_32768_blocked_inverse(
    mat: torch.Tensor,
) -> torch.Tensor:
    global _BLOCKED_INV_32768_HITS
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if is_f32_cuda and batch == 1 and n in _LARGE_CFG:
        try:
            if n == 32768:
                l = _left_looking_32768_blocked_inverse(data[0])
            else:
