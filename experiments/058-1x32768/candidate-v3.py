...
# ---------------------------------------------------------------------------
# Experiment 058 V3: blocked inverse plus MXFP8 panel application at 1x32768.
#
...
# each tree level with batched GEMMs. Apply the panel through that inverse using
# the same native block-scaled MXFP8 tensor-core route as the left-looking
# update, with FP32 output. The panel width and every other dispatch stay fixed.
# ---------------------------------------------------------------------------
_BLOCKED_INV_32768_HITS = 0
_MXFP8_SOLVE_32768_HITS = 0
...
def _mxfp8_solve_32768(
    panel: torch.Tensor, inverse: torch.Tensor
) -> torch.Tensor:
    global _MXFP8_SOLVE_32768_HITS

    q_panel, s_panel = _mx_quant_e4m3_blocked(panel)
    q_inverse, s_inverse = _mx_quant_e4m3_blocked(inverse)
    solved = torch._scaled_mm(
        q_panel,
        q_inverse.t(),
        scale_a=s_panel,
        scale_b=s_inverse,
...
def _left_looking_32768_blocked_inverse(
    ...
    try:
        ...
        for k in range(0, n, nb):
            ...
            factor[j:, k : k + kb] = _mxfp8_solve_32768(
                panel, inverse
            )
    finally:
        torch.backends.cuda.matmul.allow_tf32 = previous_tf32
