...
EXP = ROOT / "experiments" / "016a-large-n-fp8"
...
def main():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=True,
        help="one or more candidate files (probed against the baseline in one process)",
    )
    parser.add_argument(
        "--baseline",
        default=None,
    ...
    candidates = [EXP / c for c in args.candidate]
    image = (
    ...
    for i, cand in enumerate(candidates, 1):
        image = image.add_local_file(
            str(cand), f"/root/candidate_{i}.py", copy=True
        )
    ...
    app = modal.App.lookup("gpumode-cholesky-exp015", create_if_missing=True)
    payload = None
