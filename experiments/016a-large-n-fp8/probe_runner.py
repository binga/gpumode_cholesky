"""Runs INSIDE the Modal B200 sandbox for experiment 015.

Paired same-process probe: times the exact ranked baseline (#880770 source)
against an exp-015 candidate on the target mid shapes, mirroring the official
...
def _probe_shape(spec, base_fn, cand_fn, base_mod, cand_mod):
    ...
    base_times, base_outs = _time_fn(base_fn, data_list)
    pre_cand = _counters(cand_mod)
    cand_times, cand_outs = _time_fn(cand_fn, data_list)
...
def _family_sweep(spec, cand_fn, cand_mod):
    ...
    fams = FAMILIES
    if spec["n"] >= 32768:
        fams = ["dense", "lowrank", "tridiagonal"]
    for fam in fams:
        args = dict(spec)
        ...
        rows.append(
            {
                ...
                "msg": msg,
                "hits_delta": sum(
                    (post.get(k) or 0) - (pre.get(k) or 0)
                    for k in ("_LEFT_16384_HITS", "_LEFT_32768_HITS", "_LARGE_FP8_HITS")
                ),
                "fallbacks_delta": sum(
                    (post.get(k) or 0) - (pre.get(k) or 0)
                    for k in ("_LEFT_LARGE_FALLBACKS", "_LARGE_FP8_FALLBACKS")
                ),
            }
        )
...
def main():
    ...
    cand_paths = sorted(_glob.glob("/root/candidate_*.py"))
    cands = [
        (path.split("/")[-1], _load(f"cand_mod_{i}", path))
        for i, path in enumerate(cand_paths)
    ]

    payload = {
        "schema_version": 1,
        "experiment": "016a-large-n-fp8",
        "mode": "paired-probe",
        "environment": {
    ...
    for spec in specs:
        label = f"{spec['batch']}x{spec['n']}"
        for cname, cand_mod in cands:
            print(f"probing {label} [{cname}] ...", flush=True)
            res = _probe_shape(
                spec, base_mod.custom_kernel, cand_mod.custom_kernel,
                base_mod, cand_mod,
            )
            ...
            if do_profile:
                ...
                payload.setdefault("profiles", {})[f"{label}:{cname}"] = prof_rows
            ...
            if not skip_families:
                ...
                payload["families"][f"{label}:{cname}"] = fams
    ...
    payload["final_counters"] = {
        cname: _counters(cand_mod) for cname, cand_mod in cands
    }
