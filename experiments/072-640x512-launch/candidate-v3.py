...
# Experiment 072: 640x512-only fusion of the small diagonal-block panel apply
# and its immediately dependent rank-32 inner update.  The three standalone
# candidates vary this tile/warp tuple; no other dispatch shape observes it.
_EXP072_FUSE_CFG = (32, 8)
_EXP072_FUSED_HITS = 0
...
if _HAVE_TRITON:
    ...
    @triton.jit
    def _panel_apply_inner32_fused(
        out_ptr,
        inv_ptr,
        src_ptr,
        n: tl.constexpr,
        ...
        """One CTA per matrix: apply once, then update column tiles serially.

        The full active panel (at most 96x32) is loaded before any factor store.
        A single CTA owns every row and column of this matrix's update, so no
        other CTA can observe the in-place factor publication early.  Segmenting
        the output columns limits the live accumulator to 128xTILE_C instead of
        the rejected `_diag_block_step` kernel's 128x128 surface.
        """
        b = tl.program_id(0).to(tl.int64)
        ...
        r_off = (k + 32 + rows)[:, None] * n + (k + d)[None, :]
        if FIRST:
            ra = tl.load(
                src_ptr + base + r_off, mask=rmask[:, None], other=0.0
            )
        else:
            ra = tl.load(
                out_ptr + base + r_off, mask=rmask[:, None], other=0.0
            )
        dinv = tl.load(
            inv_ptr + b * 1024 + d[:, None] * 32 + d[None, :]
        )
        lik = tl.dot(
            ra, tl.trans(dinv), input_precision=PREC, out_dtype=tl.float32
        )
        for start in tl.static_range(0, 128, TILE_C):
            cols = start + tl.arange(0, TILE_C)
            cmask = cols < width
            c_off = (k + 32 + cols)[:, None] * n + (k + d)[None, :]
            if FIRST:
                ca = tl.load(
                    src_ptr + base + c_off, mask=cmask[:, None], other=0.0
                )
            else:
                ca = tl.load(
                    out_ptr + base + c_off, mask=cmask[:, None], other=0.0
                )
            ljk = tl.dot(
                ca, tl.trans(dinv), input_precision=PREC, out_dtype=tl.float32
            )
            t_off = (
                (k + 32 + rows)[:, None] * n
                + (k + 32 + cols)[None, :]
            ...
            if FIRST:
                ...
                t = tl.load(out_ptr + base + t_off, mask=valid, other=0.0)
            prod = tl.dot(
                lik, tl.trans(ljk), input_precision=PREC, out_dtype=tl.float32
            )
            tl.store(out_ptr + base + t_off, t - prod, mask=valid)

        # Publish the factors only after all column-source tiles were consumed.
        tl.store(out_ptr + base + r_off, lik, mask=rmask[:, None])
        mirror = (
            out_ptr
            + base
            + (k + d)[:, None] * n
    ...
    def _split32_launch(
        ...
        global _FUSED_PANEL_HITS, _EXP072_FUSED_HITS
        ...
        for nb in _nb_schedule(batch, n):
            ...
            for k in range(j, panel_end, 32):
                ...
                exp072_fuse = (batch, n) == (640, 512) and width > 0
                if exp072_fuse:
                    fc, fw = _EXP072_FUSE_CFG
                    _EXP072_FUSED_HITS += 1
                    _panel_apply_inner32_fused[(batch,)](
                        work,
                        dinv[slot],
                        src,
                        n=n,
