...
# experiment 013: cuSOLVER-free 1x32768 diagnostics. The diagonal block potrf is
# factored by a Triton blocked scheme (no torch.linalg.cholesky/cholesky_ex on
# the fast path). These counters let a paired probe prove the cuSOLVER-free path
# actually ran with zero safety-net fallbacks.
_NOCUSOLVER_32768_HITS = 0
_NOCUSOLVER_POTRF_CALLS = 0
# Tunables the paired probe can override via setattr before timing.
_DIAG_POTRF_BK = 128
_DIAG_POTRF_TILE = 128
_PANEL_SOLVE_FP8 = True
_DIAG_METHOD = "triton"
...
def _triton_blocked_potrf(block: torch.Tensor) -> torch.Tensor:
    """cuSOLVER-free Cholesky of one contiguous (m, m) FP32 SPD block.

    Right-looking blocked scheme reusing the validated experiment-009 Triton
    kernels: the small `bk x bk` diagonal tile is factored inside a single Triton
    program (unblocked, FP32), the panel below it is solved by an exact-division
    Triton kernel (FP32), and the O(m^3) trailing update runs on Blackwell tensor
    cores in TF32. No torch.linalg.cholesky/cholesky_ex is used. Operates in place
    on `block` (assumed already cloned/contiguous) and returns the lower factor.
    """
    global _NOCUSOLVER_POTRF_CALLS
    _NOCUSOLVER_POTRF_CALLS += 1
    out = block
    m = out.shape[0]
    bk = _DIAG_POTRF_BK
    tile = _DIAG_POTRF_TILE
    previous_tf32 = torch.backends.cuda.matmul.allow_tf32
    try:
        torch.backends.cuda.matmul.allow_tf32 = True
        for k in range(0, m, bk):
            _diag_factor_8x2048[(1,)](
                out,
                n=m,
                k=k,
                BK_CONST=bk,
            ...
            if remaining <= 0:
                break
            panel_tiles = triton.cdiv(remaining, bk)
            _panel_solve_8x2048[(panel_tiles, 1)](
                out,
                n=m,
                k=k,
                remaining=remaining,
            ...
            update_tiles = triton.cdiv(remaining, tile)
            triangular_tiles = update_tiles * (update_tiles + 1) // 2
            _lower_schur_8x2048[(triangular_tiles, 1)](
                out,
                n=m,
                k=k,
                remaining=remaining,
...
def _blocked_cublas_potrf(block: torch.Tensor, bk: int, use_tf32: bool) -> torch.Tensor:
    """cuSOLVER-free right-looking blocked Cholesky of a contiguous (m, m) FP32
    SPD block. The `bk x bk` diagonal tile is factored by the Triton n=32 kernel
    (bk must be 32); the panel solve uses cuBLAS trsm (solve_triangular) and the
    trailing update uses a cuBLAS matmul (TF32 or FP32). No cuSOLVER."""
    global _NOCUSOLVER_POTRF_CALLS
    _NOCUSOLVER_POTRF_CALLS += 1
    a = block
    m = a.shape[0]
    ...
    try:
        ...
        for k in range(0, m, bk):
            a11 = a[k : k + bk, k : k + bk].contiguous()
            l11 = _triton_cholesky32(a11.unsqueeze(0))[0]
            a[k : k + bk, k : k + bk] = l11
            j = k + bk
            if j >= m:
                break
...
def _left_looking_cholesky_32768(mat: torch.Tensor) -> torch.Tensor:
    global _LEFT_32768_HITS, _NOCUSOLVER_32768_HITS
    ...
    try:
        ...
        for k in range(0, n, nb):
            ...
            # cuSOLVER-free diagonal factorization (experiment 013): the FP32
            # diagonal potrf becomes tensor-core Triton work instead of a
            # cusolverDnSpotrf call.
            if _DIAG_METHOD == "cublas32_fp32":
                diagonal_factor = _blocked_cublas_potrf(diagonal, 32, False)
            elif _DIAG_METHOD == "cublas32_tf32":
                diagonal_factor = _blocked_cublas_potrf(diagonal, 32, True)
            else:
                diagonal_factor = _triton_blocked_potrf(diagonal)
            ...
            # Panel solve as a matmul against the explicit triangular inverse.
            # experiment 013 pushes this large O(n^3) product onto native FP8
            # Blackwell tensor cores (it was TF32 in exp 012).
            if _PANEL_SOLVE_FP8:
                factor[j:, k : k + kb] = _fp8_product_32768(
                    panel, inverse_transpose
                )
            else:
