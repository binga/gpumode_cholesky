...
# ---------------------------------------------------------------------------
# Experiment 038: cuSOLVER-free two-CTA cluster Cholesky for 2x2048.
#
# One hardware cluster owns each matrix.  Rank 0 factors a 16x16 diagonal tile
# in shared memory; both CTAs consume that tile through distributed shared
# memory, solve their share of the panel, and update lower-triangular 16x16
# tiles with TF32 WMMA / FP32 accumulation.  cluster.sync() replaces the global
# spin barriers that set experiment 028's ~2.75ms floor.
# ---------------------------------------------------------------------------
_CLUSTER_CUDA_SRC = r"""
...
cluster_cholesky_2048(float* __restrict__ matrices) {
    __shared__ float diag[PANEL * PANEL];
    cg::cluster_group cluster = cg::this_cluster();
    const int rank = static_cast<int>(cluster.block_rank());
    const int matrix = static_cast<int>(blockIdx.x) / 2;
    float* a = matrices + static_cast<size_t>(matrix) * N * N;

    // Establish the distributed-shared-memory mapping before rank 1 accesses
    // rank 0's diagonal tile.
    cluster.sync();
    ...
    for (int k = 0; k < N; k += PANEL) {
        if (rank == 0) {
            for (int idx = threadIdx.x; idx < PANEL * PANEL; idx += blockDim.x) {
                ...
                diag[idx] = a[(k + r) * N + k + c];
            }
            __syncthreads();

            // The diagonal recurrence is genuinely serial.  Keeping it on one
            // lane avoids 16 block-wide barriers; all parallel work follows in
            // the panel solve and WMMA Schur update.
            if (threadIdx.x == 0) {
                #pragma unroll
                for (int j = 0; j < PANEL; ++j) {
                    float d = diag[j * PANEL + j];
                    #pragma unroll
                    for (int p = 0; p < j; ++p) {
                        const float v = diag[j * PANEL + p];
                        d = fmaf(-v, v, d);
                    }
                    d = sqrtf(d);
                    diag[j * PANEL + j] = d;
                    #pragma unroll
                    for (int i = j + 1; i < PANEL; ++i) {
                        float v = diag[i * PANEL + j];
                        #pragma unroll
                        for (int p = 0; p < j; ++p) {
                            v = fmaf(-diag[i * PANEL + p], diag[j * PANEL + p], v);
                        }
                        diag[i * PANEL + j] = v / d;
                    }
                }
            ...
            for (int idx = threadIdx.x; idx < PANEL * PANEL; idx += blockDim.x) {
                const int r = idx / PANEL;
                const int c = idx - r * PANEL;
                a[(k + r) * N + k + c] = diag[idx];
            }
        }

        cluster.sync();
        float* dsm_diag = cluster.map_shared_rank(diag, 0);
        const int first = k + PANEL;
        if (first >= N) {
            break;
        }

        // L21 = A21 * inv(L11^T).  One thread owns one row so the short
        // forward recurrence stays register-resident.
        const int cluster_thread = rank * blockDim.x + threadIdx.x;
        for (int row = first + cluster_thread; row < N; row += 2 * blockDim.x) {
            float values[PANEL];
            #pragma unroll
            for (int j = 0; j < PANEL; ++j) {
                float v = a[row * N + k + j];
                #pragma unroll
                for (int p = 0; p < j; ++p) {
                    v = fmaf(-values[p], dsm_diag[j * PANEL + p], v);
                }
                values[j] = v / dsm_diag[j * PANEL + j];
                a[row * N + k + j] = values[j];
            }
        }
        cluster.sync();

        // A22 -= L21 @ L21^T.  The lower triangle is tiled linearly across
        // all 16 warps in the two-CTA cluster.  TF32 inputs with FP32
        // accumulation match the already-validated split32 precision rung.
        const int nt = (N - first) / PANEL;
        const int tiles = nt * (nt + 1) / 2;
        const int warp = threadIdx.x / 32;
        const int cluster_warp = rank * WARPS + warp;
        for (int tile = cluster_warp; tile < tiles; tile += 2 * WARPS) {
            int br = static_cast<int>((sqrtf(8.0f * tile + 1.0f) - 1.0f) * 0.5f);
            while ((br + 1) * (br + 2) / 2 <= tile) ++br;
            while (br * (br + 1) / 2 > tile) --br;
            const int bc = tile - br * (br + 1) / 2;
            const int row0 = first + br * PANEL;
            const int col0 = first + bc * PANEL;

            wmma::fragment<wmma::accumulator, 16, 16, 8, float> accum;
            wmma::load_matrix_sync(accum, a + row0 * N + col0, N, wmma::mem_row_major);

            #pragma unroll
            for (int q = 0; q < PANEL; q += 8) {
                wmma::fragment<wmma::matrix_a, 16, 16, 8,
                               wmma::precision::tf32, wmma::row_major> lhs;
                wmma::fragment<wmma::matrix_b, 16, 16, 8,
                               wmma::precision::tf32, wmma::col_major> rhs;
                wmma::load_matrix_sync(lhs, a + row0 * N + k + q, N);
                wmma::load_matrix_sync(rhs, a + col0 * N + k + q, N);
                #pragma unroll
                for (int e = 0; e < lhs.num_elements; ++e) {
                    lhs.x[e] = -lhs.x[e];
                }
                wmma::mma_sync(accum, lhs, rhs, accum);
            }
            wmma::store_matrix_sync(a + row0 * N + col0, accum, N, wmma::mem_row_major);
        }
        cluster.sync();
    }
}

// Variant 2: the cluster owns only a 128-wide diagonal superpanel.  The
// full-matrix panel product and Schur update are separate full-grid Triton
// kernels, so the cubic work is no longer restricted to four SMs total.
constexpr int SUPER = 128;

__global__ void __cluster_dims__(2, 1, 1)
cluster_diag128(float* __restrict__ matrices,
    ...
    const int matrix = static_cast<int>(blockIdx.x) / 2;
    float* a = matrices + static_cast<size_t>(matrix) * N * N;
    float* inverse = inverses + static_cast<size_t>(matrix) * SUPER * SUPER;
    cluster.sync();

    for (int sub = 0; sub < SUPER; sub += PANEL) {
        const int pivot = k + sub;
        if (rank == 0) {
            for (int idx = threadIdx.x; idx < PANEL * PANEL; idx += blockDim.x) {
                const int r = idx / PANEL;
                const int c = idx - r * PANEL;
                diag[idx] = a[(pivot + r) * N + pivot + c];
            }
            __syncthreads();
            if (threadIdx.x == 0) {
                #pragma unroll
            ...
            for (int idx = threadIdx.x; idx < PANEL * PANEL; idx += blockDim.x) {
                const int r = idx / PANEL;
                const int c = idx - r * PANEL;
                a[(pivot + r) * N + pivot + c] = diag[idx];
            }
        }

        cluster.sync();
        float* dsm_diag = cluster.map_shared_rank(diag, 0);
        const int first = sub + PANEL;
        const int cluster_thread = rank * blockDim.x + threadIdx.x;
        for (int row = first + cluster_thread; row < SUPER;
             row += 2 * blockDim.x) {
            float values[PANEL];
            #pragma unroll
            for (int j = 0; j < PANEL; ++j) {
                float v = a[(k + row) * N + pivot + j];
                #pragma unroll
                for (int p = 0; p < j; ++p) {
                    v = fmaf(-values[p], dsm_diag[j * PANEL + p], v);
                }
                values[j] = v / dsm_diag[j * PANEL + j];
                a[(k + row) * N + pivot + j] = values[j];
            }
        }
        cluster.sync();

        const int nt = (SUPER - first) / PANEL;
        const int tiles = nt * (nt + 1) / 2;
        const int warp = threadIdx.x / 32;
        const int cluster_warp = rank * WARPS + warp;
        for (int tile = cluster_warp; tile < tiles; tile += 2 * WARPS) {
            ...
            while (br * (br + 1) / 2 > tile) --br;
            const int bc = tile - br * (br + 1) / 2;
            const int row0 = k + first + br * PANEL;
            const int col0 = k + first + bc * PANEL;
            wmma::fragment<wmma::accumulator, 16, 16, 8, float> accum;
            wmma::load_matrix_sync(accum, a + row0 * N + col0, N, wmma::mem_row_major);
            #pragma unroll
            for (int q = 0; q < PANEL; q += 8) {
                ...
                wmma::fragment<wmma::matrix_b, 16, 16, 8,
                               wmma::precision::tf32, wmma::col_major> rhs;
                wmma::load_matrix_sync(lhs, a + row0 * N + pivot + q, N);
                wmma::load_matrix_sync(rhs, a + col0 * N + pivot + q, N);
                #pragma unroll
                for (int e = 0; e < lhs.num_elements; ++e) lhs.x[e] = -lhs.x[e];
                wmma::mma_sync(accum, lhs, rhs, accum);
            }
            wmma::store_matrix_sync(a + row0 * N + col0, accum, N,
                                    wmma::mem_row_major);
        }
        cluster.sync();
    }

    // Columns of a lower-triangular inverse are independent forward solves.
    const int column = rank * blockDim.x + threadIdx.x;
    if (column < SUPER) {
        #pragma unroll 1
        for (int row = 0; row < SUPER; ++row) {
            if (row < column) {
                inverse[row * SUPER + column] = 0.0f;
                continue;
            }
            float value = (row == column) ? 1.0f : 0.0f;
            for (int p = column; p < row; ++p) {
                value = fmaf(-a[(k + row) * N + k + p],
                             inverse[p * SUPER + column], value);
            }
            inverse[row * SUPER + column] =
                value / a[(k + row) * N + k + row];
        }
    }
    cluster.sync();
}

void cluster_cholesky_2048_launch(torch::Tensor matrices) {
    TORCH_CHECK(matrices.is_cuda(), "cluster_cholesky_2048: CUDA tensor required");
    TORCH_CHECK(matrices.scalar_type() == torch::kFloat32,
                "cluster_cholesky_2048: float32 required");
    TORCH_CHECK(matrices.is_contiguous(), "cluster_cholesky_2048: contiguous required");
    TORCH_CHECK(matrices.size(1) == N && matrices.size(2) == N,
                "cluster_cholesky_2048: n=2048 required");
    const int blocks = static_cast<int>(matrices.size(0)) * 2;
    cluster_cholesky_2048<<<blocks, THREADS>>>(matrices.data_ptr<float>());
    const cudaError_t error = cudaPeekAtLastError();
    TORCH_CHECK(error == cudaSuccess, "cluster_cholesky_2048 launch failed: ",
                cudaGetErrorString(error));
}

void cluster_diag128_launch(torch::Tensor matrices,
                            torch::Tensor inverses,
                            int64_t k) {
    TORCH_CHECK(matrices.is_cuda() && inverses.is_cuda(),
                "cluster_diag128: CUDA tensors required");
    TORCH_CHECK(matrices.is_contiguous() && inverses.is_contiguous(),
                "cluster_diag128: contiguous tensors required");
    const int blocks = static_cast<int>(matrices.size(0)) * 2;
    cluster_diag128<<<blocks, THREADS>>>(matrices.data_ptr<float>(),
                                        inverses.data_ptr<float>(),
                                        static_cast<int>(k));
    const cudaError_t error = cudaPeekAtLastError();
    TORCH_CHECK(error == cudaSuccess, "cluster_diag128 launch failed: ",
                cudaGetErrorString(error));
}
"""
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA_MOD = load_inline(
            name="chol_cluster_2048_v2",
            cpp_sources=(
                "void cluster_cholesky_2048_launch(torch::Tensor matrices);"
                "void cluster_diag128_launch(torch::Tensor matrices, "
                "torch::Tensor inverses, int64_t k);"
            ),
            cuda_sources=_CLUSTER_CUDA_SRC,
            functions=["cluster_cholesky_2048_launch", "cluster_diag128_launch"],
            extra_cuda_cflags=["-O3", "--use_fast_math"],
            verbose=False,
        )
    except Exception as exc:  # pragma: no cover - reported as invalid fallback
        import traceback

        _CUDA_MOD = None
        _CUDA_LOAD_ERROR = "".join(traceback.format_exception_only(type(exc), exc))

_CLUSTER_2048_HITS = 0
_CLUSTER_2048_FALLBACKS = 0


def _cluster_cholesky_2x2048(data: torch.Tensor) -> torch.Tensor:
    global _CLUSTER_2048_HITS
    out = data.contiguous().clone()
    _CUDA_MOD.cluster_cholesky_2048_launch(out)
    nt = triton.cdiv(out.shape[-1], 64)
    _clear_upper_tiles[(nt * (nt + 1) // 2, out.shape[0])](
        out, n=out.shape[-1], TILE=64, num_warps=4
    )
    _CLUSTER_2048_HITS += 1
    return out


_CLUSTER128_INV = None


def _cluster128_cholesky_2x2048(data: torch.Tensor) -> torch.Tensor:
    global _CLUSTER_2048_HITS, _CLUSTER128_INV
    out = data.contiguous().clone()
    if _CLUSTER128_INV is None:
        _CLUSTER128_INV = torch.empty(
            (2, 128, 128), device=data.device, dtype=torch.float32
        )
    inverse = _CLUSTER128_INV
    n = out.shape[-1]
    for k in range(0, n, 128):
        _CUDA_MOD.cluster_diag128_launch(out, inverse, k)
        remaining = n - k - 128
        if remaining <= 0:
            break
        _panel_apply128[(triton.cdiv(remaining, 128), 2)](
            out,
            inverse,
            n=n,
            k=k,
        ...
        tr = triton.cdiv(remaining, 128)
        _trailing_nb[(tr * (tr + 1) // 2, 2)](
            out,
            out,
            n=n,
            j=k,
    ...
    nt = triton.cdiv(n, 64)
    _clear_upper_tiles[(nt * (nt + 1) // 2, 2)](
        out, n=n, TILE=64, num_warps=4
    )
    _CLUSTER_2048_HITS += 1
    return out
...
if _HAVE_TRITON:
    ...
    def _panel_apply128(
        ...
        row_tile = tl.program_id(0)
        b = tl.program_id(1).to(tl.int64)
        rows = row_tile * TILE_R + tl.arange(0, TILE_R)
        cols = tl.arange(0, 128)
        depth = tl.arange(0, 128)
        panel = tl.load(
            out_ptr
            + b * n * n
            + (k + 128 + rows)[:, None] * n
        ...
        inverse = tl.load(
            inv_ptr + b * 128 * 128 + cols[:, None] * 128 + depth[None, :]
        )
        solved = tl.dot(
            panel,
            tl.trans(inverse),
...
def custom_kernel(data: input_t) -> output_t:
    ...
    global _CLUSTER_2048_FALLBACKS
    ...
    if is_f32_cuda and batch == 2 and n == 2048:
        if _CUDA_MOD is not None and _HAVE_TRITON:
            return _cluster128_cholesky_2x2048(data)
        _CLUSTER_2048_FALLBACKS += 1
