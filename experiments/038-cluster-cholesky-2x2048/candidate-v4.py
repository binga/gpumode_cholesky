...
cluster_diag128(float* __restrict__ matrices,
    ...
    for (int sub = 0; sub < SUPER; sub += PANEL) {
        ...
        const int cluster_thread = rank * 32 + threadIdx.x;
        for (int row = first + cluster_thread;
             threadIdx.x < 32 && row < SUPER; row += 64) {
            float values[PANEL];
            #pragma unroll
            for (int j = 0; j < PANEL; ++j) {
    ...
    // Variant 3 deliberately stops after factorization.  The tall panel is
    // solved by the tuned batched cuBLAS TRSM path outside this kernel; this
    // isolates the 128-column scalar inverse that dominated Variant 2.
}
...
def _cluster128_cholesky_2x2048(data: torch.Tensor) -> torch.Tensor:
    ...
    if _CLUSTER128_INV is None:
        _CLUSTER128_INV = torch.empty(
            (2, 64, 64), device=data.device, dtype=torch.float32
        )
    ...
    for k in range(0, n, 64):
        ...
        if remaining <= 0:
            break
        diagonal = out[:, k : k + 64, k : k + 64]
        panel = out[:, k + 64 :, k : k + 64]
