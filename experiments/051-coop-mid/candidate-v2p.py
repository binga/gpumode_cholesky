...
// Experiment 051 phase probe: %globaltimer is a grid-wide nanosecond clock, so
// one designated thread can attribute wall time to phases that are separated by
// grid-wide barriers. Guarded by a null pointer so the production launch is
// unperturbed apart from one predictable branch per phase.
...
__global__ __launch_bounds__(COOP_THREADS, COOP_MIN_BLOCKS) void coop_cholesky_v2(
    ...
    const bool timer = (prof != nullptr && blockIdx.x == 0 && thread == 0);
    unsigned long long acc[4] = {0, 0, 0, 0};
    unsigned long long mark = timer ? gtime_ns() : 0ull;
    ...
    for (int jb = 0; jb < n; jb += COOP_NB) {
        ...
        for (int kt = jb / 32; kt < panel_end_tile; ++kt) {
            ...
            if (timer) { const unsigned long long t = gtime_ns(); acc[0] += t - mark; mark = t; }
...
#pragma unroll
            ...
            grid.sync();
            if (timer) { const unsigned long long t = gtime_ns(); acc[1] += t - mark; mark = t; }
            ...
            grid.sync();
            if (timer) { const unsigned long long t = gtime_ns(); acc[2] += t - mark; mark = t; }
        }
...
#pragma unroll
        ...
        grid.sync();
        if (timer) { const unsigned long long t = gtime_ns(); acc[3] += t - mark; mark = t; }
    }

    if (timer) {
        prof[0] = acc[0]; prof[1] = acc[1]; prof[2] = acc[2]; prof[3] = acc[3];
    }
...
static void coop051_run(torch::Tensor matrix, unsigned long long* prof) {
...
void coop051_launch(torch::Tensor matrix) { coop051_run(matrix, nullptr); }

torch::Tensor chol4096_profile(torch::Tensor matrix) {
    auto buffer = torch::zeros(
        {4}, torch::TensorOptions().dtype(torch::kInt64).device(matrix.device()));
    coop051_run(matrix,
                reinterpret_cast<unsigned long long*>(buffer.data_ptr<int64_t>()));
    return buffer.to(torch::kCPU);
}
"""
    ...
    try:
        ...
        _CUDA_ALL = load_inline(
            ...
            cpp_sources=(
                ...
                "int64_t coop051_ctas(int64_t, int64_t);"
...
_COOP_FALLBACKS = 0
_COOP_ERROR = None
_COOP_SHAPES = {(4, 1024), (16, 512), (8, 2048), (2, 2048), (1, 4096), (2, 4096)}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    global _COOP_HITS, _COOP_FALLBACKS, _COOP_ERROR
    ...
    # Experiment 051: single-launch cooperative tile-32 factorization for the
