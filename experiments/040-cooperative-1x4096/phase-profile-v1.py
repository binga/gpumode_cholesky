"""Instrumented Experiment 040 V1 for in-kernel constituent timing."""
...
__device__ unsigned long long phase_cycles[4];
...
__global__ void cooperative_cholesky_4096(float* matrix) {
    ...
    if (blockIdx.x == 0 && thread == 0) {
        #pragma unroll
        for (int phase = 0; phase < 4; ++phase) phase_cycles[phase] = 0;
    }
    grid.sync();
    ...
    for (int kt = 0; kt < NT; ++kt) {
        ...
        grid.sync();
        if (blockIdx.x == 0 && thread == 0) {
            phase_cycles[0] += global_time_ns() - phase_begin;
        ...
        grid.sync();
        if (blockIdx.x == 0 && thread == 0) {
            phase_cycles[1] += global_time_ns() - phase_begin;
            phase_begin = global_time_ns();
        }
        ...
        grid.sync();
        if (blockIdx.x == 0 && thread == 0) {
            phase_cycles[2] += global_time_ns() - phase_begin;
        }
    }
    ...
    grid.sync();
    if (blockIdx.x == 0 && thread == 0) {
        phase_cycles[3] = global_time_ns() - clear_begin;
    }
}
...
std::vector<int64_t> chol4096_profile(torch::Tensor matrix) {
    chol4096_launch(matrix);
    cudaError_t status = cudaDeviceSynchronize();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
    unsigned long long values[4];
    status = cudaMemcpyFromSymbol(values, phase_cycles, sizeof(values));
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
    return {
        (int64_t)values[0], (int64_t)values[1],
...
if torch.cuda.is_available():
    try:
        ...
        _COOP4096 = load_inline(
            name="chol4096_exp040_phase_v1",
            cpp_sources="""
#include <torch/extension.h>
#include <vector>
void chol4096_launch(torch::Tensor);
std::vector<int64_t> chol4096_profile(torch::Tensor);
""",
            cuda_sources=_COOP4096_SOURCE,
            functions=["chol4096_launch", "chol4096_profile"],
