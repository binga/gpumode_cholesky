"""Experiment 040 V3: tile-32 cooperative Cholesky with MMA panel solves."""
...
__device__ float inverse_tile[32 * 32];
...
__global__ void cooperative_cholesky_4096(float* matrix) {
    ...
    for (int kt = 0; kt < NT; ++kt) {
        ...
        if (blockIdx.x == 0 && warp == 0) {
            ...
            __syncwarp();
            // Build inv(Lkk) one row at a time. It is reused by every panel
            // CTA as the right-hand tensor-core operand.
            #pragma unroll
            for (int row = 0; row < 32; ++row) {
                ...
                for (int p = 0; p < 32; ++p) {
                    if (p < row) {
                        value = fmaf(
                            -tile[row][p], diagonal[p][lane], value);
                    }
                }
                diagonal[row][lane] = value / tile[row][row];
                __syncwarp();
            }
            #pragma unroll
            for (int item = 0; item < 32; ++item) {
                ...
                matrix[(size_t)(k0 + (linear >> 5)) * N + k0 + (linear & 31)] =
                    tile[linear >> 5][linear & 31];
                inverse_tile[linear] = diagonal[linear >> 5][linear & 31];
            }
        }
        grid.sync();

        // Phase 2: four warps apply inv(Lkk)^T to each 32x32 panel tile.
        for (int bi = kt + 1 + blockIdx.x; bi < NT; bi += gridDim.x) {
            ...
            float* cptr = matrix
                ...
                + k0 + warp_column * 16;
            const float* aptr = &tile[warp_row * 16][0];
            const float* bptr = inverse_tile + warp_column * 16 * 32;
            ...
            for (int depth = 0; depth < 32; depth += 8) {
                ...
                wmma::load_matrix_sync(lhs, aptr + depth, 36);
                wmma::load_matrix_sync(rhs, bptr + depth, 32);
