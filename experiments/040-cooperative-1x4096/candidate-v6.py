"""Experiment 040 V6: cooperative rank-128 superpanel Cholesky."""
...
__global__ void cooperative_cholesky_4096(float* matrix) {
    ...
    for (int kt = 0; kt < NT; ++kt) {
        ...
        // Phase 3: within each four-tile superpanel, update only the narrow
        // columns needed by the next microfactorization. After the fourth
        // tile, apply one rank-128 update to the far trailing matrix.
        const bool finish_superpanel = (kt & 3) == 3;
        const int panel_end = ((kt >> 2) + 1) * 4;
        int inner_pairs = 0;
        if (!finish_superpanel) {
            #pragma unroll
            for (int column_tile = 1; column_tile < 4; ++column_tile) {
                const int bj_candidate = kt + column_tile;
                if (bj_candidate < panel_end) inner_pairs += NT - bj_candidate;
            }
        }
        const int remaining_tiles = NT - panel_end;
        const int trailing_pairs =
            remaining_tiles * (remaining_tiles + 1) / 2;
        const int task_count = finish_superpanel ? trailing_pairs : inner_pairs;
        for (int task = blockIdx.x; task < task_count; task += gridDim.x) {
            int bi = 0;
            int bj = 0;
            if (finish_superpanel) {
                const int local_row = (int)(
                    (sqrtf(8.0f * task + 1.0f) - 1.0f) * 0.5f);
                const int local_column =
                    task - local_row * (local_row + 1) / 2;
                bi = panel_end + local_row;
                bj = panel_end + local_column;
            } else {
                int residual = task;
                #pragma unroll
                for (int column_tile = 1; column_tile < 4; ++column_tile) {
                    const int bj_candidate = kt + column_tile;
                    if (bj_candidate < panel_end) {
                        const int count = NT - bj_candidate;
                        if (residual < count && bj == 0) {
                            bj = bj_candidate;
                            bi = bj_candidate + residual;
                        }
                        residual -= residual < count ? 0 : count;
                    }
                }
            }
            ...
            const int update_k0 = finish_superpanel ? (kt - 3) * TILE : k0;
            const int update_depth = finish_superpanel ? 128 : 32;
            const float* aptr = matrix
                + (size_t)(bi * TILE + warp_row * 16) * N + update_k0;
            const float* bptr = matrix
                + (size_t)(bj * TILE + warp_column * 16) * N + update_k0;
            wmma::fragment<wmma::accumulator, 16, 16, 8, float> accumulator;
            wmma::fill_fragment(accumulator, 0.0f);
            for (int depth = 0; depth < update_depth; depth += 8) {
