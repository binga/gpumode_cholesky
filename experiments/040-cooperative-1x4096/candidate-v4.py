"""Experiment 040 V4: residency-saturated cooperative tile-32 Cholesky."""
...
void chol4096_launch(torch::Tensor matrix) {
    ...
    cudaError_t occupancy_status = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &active_blocks, cooperative_cholesky_4096, 128, 0);
    TORCH_CHECK(
        occupancy_status == cudaSuccess && active_blocks > 0,
        "cooperative occupancy query failed");
    const int blocks_per_sm = active_blocks < 4 ? active_blocks : 4;
    cudaError_t status = cudaLaunchCooperativeKernel(
        (void*)cooperative_cholesky_4096,
        dim3(properties.multiProcessorCount * blocks_per_sm),
        dim3(128), args, 0);
