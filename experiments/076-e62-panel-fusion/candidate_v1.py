...
// Experiment 076 V1 (Fu/Ov lever): write the scaled panel from the contiguous
// scratch `Pan` back into the strided work panel A[:, col0+128:, col0:col0+128].
// One 32-thread block per (matrix, panel row); each thread moves a float4, so
// the 128-wide row is one fully coalesced 512-byte transaction on both sides.
// Pure data movement -- the values came from the unchanged cuBLAS TF32 bmm, so
// the L factor is byte-identical to the shipped `src.copy_(dst)`.
__global__ void e62_panel_copy_v1(float* __restrict__ A,
                                  const float* __restrict__ Pan,
                                  int n, int col0, int rows)
{
    const int i = blockIdx.x;               // panel row 0..rows-1
    const int b = blockIdx.y;               // matrix in batch
    const int t = threadIdx.x;              // 0..31, one float4 each
    ...
    reinterpret_cast<float4*>(A + dst_off)[t] =
        reinterpret_cast<const float4*>(Pan + ((long long)b * rows + i) * 128LL)[t];
}
                           ...
                           int64_t n, int64_t col0, int64_t rows, int64_t batch)
{
    ...
    dim3 grid((unsigned int)rows, (unsigned int)batch);
    e62_panel_copy_v1<<<grid, 32>>>(
