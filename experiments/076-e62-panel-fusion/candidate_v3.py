...
// Experiment 076 V3 (Fu/Ov lever): scalar (non-vectorized) control for the
// byte-identical panel write-back. One 128-thread block per (matrix, panel
// row); thread t moves one float. Isolates the float4 vectorization benefit of
// V1/V2 -- if V3 already saturates the copy, the alignment path adds nothing.
__global__ void e62_panel_copy_v3(float* __restrict__ A,
                                  const float* __restrict__ Pan,
                                  int n, int col0, int rows)
{
    const int i = blockIdx.x;
    const int b = blockIdx.y;
    const int t = threadIdx.x;              // 0..127, one float each
    const long long dst_off =
        (long long)b * (long long)n * (long long)n
        + (long long)(col0 + 128 + i) * (long long)n + (long long)col0;
    A[dst_off + t] = Pan[((long long)b * rows + i) * 128LL + t];
}
