...
// Experiment 077 (stacks exp 076's Fu/Ov panel write-back with a faster
// byte-identical copy-in clone). Both keep L byte-identical: the panel values
// are produced by the unchanged cuBLAS TF32 bmm, and the clone is a flat
// contiguous copy. Fall back to torch's paths if a recompiled symbol is missing.
__global__ void e62_panel_copy_v2(float* __restrict__ A,
                                  const float* __restrict__ Pan,
                                  int n, int col0, int rows, int batch)
{
    const long long total4 = (long long)batch * (long long)rows * 32LL;
    const long long stride = (long long)gridDim.x * (long long)blockDim.x;
    for (long long idx = (long long)blockIdx.x * (long long)blockDim.x
             + (long long)threadIdx.x;
         idx < total4; idx += stride) {
        const int t = (int)(idx & 31LL);
        const long long rc = idx >> 5;
        const int i = (int)(rc % (long long)rows);
        const int b = (int)(rc / (long long)rows);
        const long long dst_off =
            (long long)b * (long long)n * (long long)n
            + (long long)(col0 + 128 + i) * (long long)n + (long long)col0;
        reinterpret_cast<float4*>(A + dst_off)[t] =
            reinterpret_cast<const float4*>(Pan + rc * 128LL)[t];
    }
}

void e62_panel_copy_launch(torch::Tensor A, torch::Tensor Pan,
                           int64_t n, int64_t col0, int64_t rows, int64_t batch)
{
    if (rows <= 0 || batch <= 0) return;
    const long long total4 = (long long)batch * (long long)rows * 32LL;
    const int block = 256;
    long long want = (total4 + block - 1) / block;
    int grid = want > 65535LL ? 65535 : (int)want;
    if (grid < 1) grid = 1;
    e62_panel_copy_v2<<<grid, block>>>(
        A.data_ptr<float>(), Pan.data_ptr<float>(),
        (int)n, (int)col0, (int)rows, (int)batch);
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
}

// Experiment 077 (Ov lever 2): a flat float4 grid-stride contiguous copy that
// replaces `data.clone()` for the e62 shapes. The shipped clone runs as a
// Memcpy DtoD at ~76% of the copy floor; an SM-filling float4 kernel trades
// the copy engine for tensor cores' memory pipe and is byte-identical for
// contiguous (batch, n, n) float32 inputs (n is always a multiple of 128 on
// the e62 shapes, so numel is a multiple of 4).
__global__ void e62_fast_clone_v2(float* __restrict__ Dst,
...
void e62_fast_clone_launch(torch::Tensor Dst, torch::Tensor Src)
{
    const long long nelem = Src.numel();
    if (nelem <= 0) return;
    if ((nelem & 3LL) != 0LL) {  // tail not float4-aligned; let torch handle it
        Dst.copy_(Src);
        return;
    }
    const long long nelem4 = nelem >> 2;
    const int block = 256;
    long long want = (nelem4 + block - 1) / block;
    int grid = want > 65535LL ? 65535 : (int)want;
    if (grid < 1) grid = 1;
    e62_fast_clone_v2<<<grid, block>>>(
        Dst.data_ptr<float>(), Src.data_ptr<float>(), nelem4);
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
}
"""
...
def _load_exp062():
    ...
    try:
        ...
        _EXP062 = load_inline(
            name="chol_exp077_panelcopy_clone",
            cpp_sources=(
                ...
                "void e62_panel_copy_launch(torch::Tensor, torch::Tensor, "
                "int64_t, int64_t, int64_t, int64_t);\n"
                "void e62_fast_clone_launch(torch::Tensor, torch::Tensor);"
            ),
            cuda_sources=_EXP062_SOURCE,
            functions=["e62_diag128_launch", "e62_diag128_launch_var",
                       "e62_zero_upper_launch", "e62_panel_copy_launch",
                       "e62_fast_clone_launch"],
...
def _exp062_factor(data, nb_outer=1024, prof=None, variant=None):
    batch, n, _ = data.shape
    # Experiment 077 (Ov lever 2): replace `data.clone()` with a flat float4
    # grid-stride contiguous copy. Byte-identical for contiguous float32 inputs
    # (the e62 shapes all have n a multiple of 128); fall back to torch's clone
    # when the recompiled symbol is unavailable or the input is non-contiguous
    # so correctness never depends on the recompile.
    _fclone = getattr(_EXP062, "e62_fast_clone_launch", None)
    if _fclone is not None and data.is_contiguous():
        work = torch.empty_like(data)
        _fclone(work, data)
    else:
        work = data.clone()
        ...
        for j0 in range(0, n, nb_outer):
            ...
            for jj in range(j0, jend, 128):
                ...
                if rows > 0:
                    ...
                    # Experiment 077 (stacks exp 076's Fu/Ov panel write-back):
                    # the scaled panel lives in the contiguous scratch `pan`;
                    # the shipped `src.copy_(dst)` writes it back to the strided
                    # work panel at ~2.6x its bandwidth floor (99us / 5.2% of
                    # 2x4096, 66us / 6.3% of 8x2048; results/077-inc-shapediag).
                    # Replace it with a coalesced float4 grid-stride write-back.
                    # The values are produced by the unchanged cuBLAS TF32 bmm,
                    # so L is byte-identical; fall back to torch's copy if the
                    # recompiled symbol is unavailable.
                    _pcopy = getattr(_EXP062, "e62_panel_copy_launch", None)
                    if _pcopy is not None:
                        _pcopy(work, pan, n, jj, rows, batch)
                    else:
                        src.copy_(dst)
            if jend < n:
