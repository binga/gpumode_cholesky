...
// Experiment 076 V2 (Fu/Ov lever): same byte-identical panel write-back as V1,
// but as a flat float4 grid-stride copy over all batch*rows*32 float4 elements
// with a fixed 256-thread block sized to fill the SMs. This trades V1's
// one-block-per-row simplicity for maximum occupancy on the many-small-row
// shapes; reads of the contiguous `Pan` are perfectly coalesced.
                                  ...
                                  int n, int col0, int rows, int batch)
{
    ...
    for (long long idx = (long long)blockIdx.x * (long long)blockDim.x
        ...
        const int t = (int)(idx & 31LL);           // float4 within the row
        const long long rc = idx >> 5;             // = b*rows + i
        ...
        for j0 in range(0, n, nb_outer):
            ...
            for jj in range(j0, jend, 128):
                ...
                if rows > 0:
                    ...
                    # Experiment 076 (Fu/Ov lever): the scaled panel lives in the
                    # contiguous scratch `pan`; the shipped `src.copy_(dst)` writes
                    # it back to the strided work panel at ~2.6x its bandwidth
                    # floor (73.6us / 8.5% of 60x1024, results/069-inc-shapediag-mid).
                    # Replace it with a coalesced write-back kernel. The values are
                    # produced by the unchanged cuBLAS TF32 bmm, so L is byte-
                    # identical; fall back to torch's copy if the recompiled symbol
                    # is unavailable so correctness never depends on the kernel.
