...
def _mxfp8_panel_update(
    ...
    """out -= lhs @ rhs^T on MXFP8 block-scaled tensor cores. lhs (M, K) and
    rhs (N, K) may be strided factor views; out must be contiguous (M, N).
    All sizes in the 32768 left-looking schedule are multiples of nb=4096, so
    exact tiling always holds; anything else raises and the caller's existing
    fallback chain takes over."""
    global _MXFP8_HITS, _MXFP8_PTX
    m_rows, k_cols = lhs.shape
    n_rows = rhs.shape[0]
    if (
        m_rows % _MX_GEMM_BLOCK_M
        or n_rows % _MX_GEMM_BLOCK_N
        or k_cols % _MX_GEMM_BLOCK_K
    ...
    q_lhs, s_lhs = _mx_quant_e4m3(lhs)
    q_rhs, s_rhs = _mx_quant_e4m3(rhs)
    handle = _mxfp8_panel_update_kernel[
        (m_rows // _MX_GEMM_BLOCK_M, n_rows // _MX_GEMM_BLOCK_N)
    ](
        q_lhs,
        s_lhs,
        q_rhs,
        ...
        n_rows,
        k_cols,
        BLOCK_M=_MX_GEMM_BLOCK_M,
        BLOCK_N=_MX_GEMM_BLOCK_N,
        BLOCK_K=_MX_GEMM_BLOCK_K,
        num_warps=_MX_GEMM_WARPS,
        num_stages=_MX_GEMM_STAGES,
    )
    if _MXFP8_PTX is None:
        try:
            _MXFP8_PTX = handle.asm.get("ptx", "")
        except Exception:
            _MXFP8_PTX = ""
    _MXFP8_HITS += 1
