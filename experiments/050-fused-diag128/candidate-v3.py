...
constexpr int D128_SHARED_BYTES =
    (D128_N * D128_LDS + 32 * D128_INV_LDS + 4 * 32) * (int)sizeof(float);
...
void diag128_potrf(const float* __restrict__ src, float* __restrict__ work,
    ...
    for (int kb = 0; kb < D128_N; kb += 32) {
        if (warp == 0) {
            ...
            for (int iteration = 0; iteration < 8; ++iteration) {
                ...
                for (int c = 0; c < 32; ++c) {
                    if (c > p3) {
                        float value = values[c];
                        if (c <= lane) {
                            value = fmaf(-s0, pivots[0 * 32 + c], value);
                            value = fmaf(-s1, pivots[1 * 32 + c], value);
                            value = fmaf(-s2, pivots[2 * 32 + c], value);
                            value = fmaf(-s3, pivots[3 * 32 + c], value);
                        }
                        values[c] = value;
                    }
                }
            ...
            #pragma unroll
            for (int c = 0; c < 32; ++c) {
                pivot_row[c] = (c <= lane) ? values[c] : 0.0f;
            }
            pivots[0 * 32 + lane] = reciprocal_row;
            __syncwarp();

            // Triangular inverse of the finished 32x32 factor: lane owns
            // column `lane` of X, row r is final after step r.
            float inverse[32];
            #pragma unroll
            for (int r = 0; r < 32; ++r) {
                const float* factor_row =
                    tile + (size_t)(kb + r) * D128_LDS + kb;
                float accumulator = (r == lane) ? 1.0f : 0.0f;
                #pragma unroll
                for (int p = 0; p < 32; ++p) {
                    if (p < r) {
                        accumulator =
                            fmaf(-factor_row[p], inverse[p], accumulator);
                    }
                }
...
_DIAG128_SHAPES = {
    ...
    (1, 4096): (128, 8),
    (2, 4096): (128, 8),
    # High-batch mid shapes: these are already eager, so enrolling them costs
    # no graph. At 640x512 the shipped schedule burns 352us of the 1480.7us
    # wall in launch idle across ~46 launches; the fused schedule emits 8.
    (640, 512): (128, 8),
    (60, 1024): (128, 8),
}
_DIAG128_HITS = 0
...
if _HAVE_TRITON:
    ...
    _SPLIT32_SHAPES = {
        ...
        (1, 4096): ("tf32", "tf32", 128, "eager", True),
