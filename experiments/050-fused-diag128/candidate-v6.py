...
//
// Experiment 050 lifted the body out of the kernel so `diag128_potrf` can
// reuse it. It is `__noinline__` on purpose: two inlined copies of this
// fully-unrolled 32-pivot chain plus its 32-step triangular inverse pushed
// the official runner's cold nvcc build past the six-minute service budget
// (tests #898552 and #898531 both stopped at exactly 360s). One shared copy
// keeps the whole submission at three nvcc invocations and one instance of
// the expensive register-allocation problem.
//
// `sub` is a 32-row shared staging tile of stride MICRO_LDS holding the block
// to factor; on return it holds the lower-triangular factor with a zeroed
// strict upper. `inv_out` already points at element [0][lane] of the caller's
// inverse destination and is written with stride `inv_lds`; it may live in
// shared or global memory.
constexpr int MICRO_LDS = MICRO_BK + 1;

__device__ __noinline__ void warp_potrf32_inv(
        float* sub, float* pivots, float* inv_out, int inv_lds, int lane) {
    float values[MICRO_BK];
    #pragma unroll
    for (int c = 0; c < MICRO_BK; ++c) values[c] = sub[lane * MICRO_LDS + c];
    ...
    for (int iteration = 0; iteration < MICRO_BK / 4; ++iteration) {
        ...
        for (int c = 0; c < MICRO_BK; ++c) {
            if (c > p3) {
                ...
                if (c <= lane) {
                    value = fmaf(-s0, pivots[0 * MICRO_BK + c], value);
                    value = fmaf(-s1, pivots[1 * MICRO_BK + c], value);
                    value = fmaf(-s2, pivots[2 * MICRO_BK + c], value);
                    value = fmaf(-s3, pivots[3 * MICRO_BK + c], value);
                }
                values[c] = value;
            }
        }
    ...
    #pragma unroll
    for (int c = 0; c < MICRO_BK; ++c) {
        sub[lane * MICRO_LDS + c] = (c <= lane) ? values[c] : 0.0f;
    }
    pivots[lane] = reciprocal_row;
    __syncwarp();
    ...
    for (int r = 0; r < MICRO_BK; ++r) {
        ...
        for (int p = 0; p < MICRO_BK; ++p) {
            if (p < r) {
                accumulator =
                    fmaf(-sub[r * MICRO_LDS + p], inverse[p], accumulator);
            }
        }
        inverse[r] = (r >= lane) ? accumulator * pivots[r] : 0.0f;
    }
    #pragma unroll
    for (int r = 0; r < MICRO_BK; ++r) inv_out[r * inv_lds] = inverse[r];
}
...
void micro_potrf32_rank4(const float* __restrict__ src, float* __restrict__ work,
    ...
    __shared__ float staging[MICRO_WARPS][MICRO_BK * MICRO_LDS];
    __shared__ float pivot_s[MICRO_WARPS][4 * MICRO_BK];
    float* sub = staging[warp];
    float* pivots = pivot_s[warp];
    ...
    for (int row = 0; row < MICRO_BK; ++row) {
        sub[row * MICRO_LDS + lane] = input[(size_t)row * n + lane];
    }
    __syncwarp();

    warp_potrf32_inv(
        sub, pivots,
        inv + (size_t)matrix * MICRO_BK * MICRO_BK + lane, MICRO_BK, lane);
    __syncwarp();

    #pragma unroll
    for (int row = 0; row < MICRO_BK; ++row) {
        output[(size_t)row * n + lane] = sub[row * MICRO_LDS + lane];
    }
}

// --- Experiment 050: fused 128x128 diagonal-block factorization.
//
// The shipped split32 schedule spends seven launches on one 128-wide diagonal
// block (4x micro potrf + 4x panel apply + 3x panel inner). Six of the seven
// touch only the 128x128 block itself, which is 66 KB -- it fits in shared
// memory, so the whole block can be factored by one CTA in one launch. The
// serial 32-pivot chains stay warp-synchronous on warp 0 (an eight-warp
// __syncthreads chain measured ~324ns/pivot against ~134ns/pivot for a single
// warp, exp 044); the other seven warps only join for the two block-parallel
// phases, which need four __syncthreads per 32-wide sub-step, not 32.
//
// Outputs are exactly what the shipped Triton panel/trailing kernels expect:
// the lower-triangular 128x128 factor written back in place (strict upper
// zeroed) and the four 32x32 triangular inverses published to `dinv` slots.
constexpr int D128_N = 128;
constexpr int D128_LDS = 129;  // odd stride: column reads hit 32 distinct banks
constexpr int D128_THREADS = 256;
constexpr int D128_WARPS = D128_THREADS / 32;
constexpr int D128_INV_LDS = 33;
constexpr int D128_SHARED_BYTES =
    (D128_N * D128_LDS + 32 * D128_INV_LDS + 4 * 32 + 32 * MICRO_LDS)
    * (int)sizeof(float);

__global__ __launch_bounds__(D128_THREADS)
void diag128_potrf(const float* __restrict__ src, float* __restrict__ work,
                   float* __restrict__ dinv, int n, int j, int first,
                   int dinv_slot_stride) {
    extern __shared__ float d128_smem[];
    float* tile = d128_smem;
    float* dinv_s = tile + D128_N * D128_LDS;
    float* pivots = dinv_s + 32 * D128_INV_LDS;
    float* sub = pivots + 4 * 32;

    const int tid = threadIdx.x;
    const int warp = tid >> 5;
    const int lane = tid & 31;
    const size_t matrix = blockIdx.x;
    const size_t base = matrix * (size_t)n * (size_t)n
                      + (size_t)j * (size_t)n + (size_t)j;
    const float* input = (first ? src : work) + base;
    float* output = work + base;

    for (int idx = tid; idx < D128_N * D128_N; idx += D128_THREADS) {
        const int row = idx >> 7;
        const int column = idx & 127;
        tile[row * D128_LDS + column] = input[(size_t)row * n + column];
    }
    __syncthreads();

    #pragma unroll 1
    for (int kb = 0; kb < D128_N; kb += 32) {
        if (warp == 0) {
            // Stage the 32x32 pivot block, factor it with the shared
            // warp-synchronous routine, then publish the factor back into the
            // resident block and the triangular inverse to both the in-block
            // panel solve (`dinv_s`) and the slot `_panel_fused128` reads.
            #pragma unroll
            for (int c = 0; c < 32; ++c) {
                sub[lane * MICRO_LDS + c] =
                    tile[(size_t)(kb + lane) * D128_LDS + kb + c];
            }
            __syncwarp();

            warp_potrf32_inv(sub, pivots, dinv_s + lane, D128_INV_LDS, lane);
            __syncwarp();
            ...
            float* inverse_out = dinv + (size_t)(kb >> 5) * dinv_slot_stride
                               + matrix * 1024 + lane;
            #pragma unroll
            for (int r = 0; r < 32; ++r) {
                inverse_out[r * 32] = dinv_s[r * D128_INV_LDS + lane];
            }
        }
        __syncthreads();

        const int below = kb + 32;
        if (below < D128_N) {
            // Panel solve inside the block: L21 = A21 @ Linv^T.
            for (int row = below + warp; row < D128_N; row += D128_WARPS) {
                const float* source = tile + (size_t)row * D128_LDS + kb;
                const float* column = dinv_s + lane * D128_INV_LDS;
                float accumulator = 0.0f;
                #pragma unroll 8
                for (int p = 0; p <= lane; ++p) {
                    accumulator = fmaf(source[p], column[p], accumulator);
                }
                __syncwarp();
                tile[(size_t)row * D128_LDS + kb + lane] = accumulator;
            }
            __syncthreads();

            // Rank-32 Schur update of the lower trailing block.
            for (int row = below + warp; row < D128_N; row += D128_WARPS) {
                const float* row_panel = tile + (size_t)row * D128_LDS + kb;
                float held[32];
                #pragma unroll
                for (int p = 0; p < 32; ++p) held[p] = row_panel[p];
                for (int column = below + lane; column <= row; column += 32) {
                    const float* column_panel =
                        tile + (size_t)column * D128_LDS + kb;
                    float update = 0.0f;
                    #pragma unroll
                    for (int p = 0; p < 32; ++p) {
                        update = fmaf(held[p], column_panel[p], update);
                    }
                    tile[(size_t)row * D128_LDS + column] -= update;
                }
            }
            __syncthreads();
        }
    ...
    for (int idx = tid; idx < D128_N * D128_N; idx += D128_THREADS) {
        ...
        output[(size_t)row * n + column] =
            column <= row ? tile[row * D128_LDS + column] : 0.0f;
    }
}
...
void diag128_launch(
    ...
    const int slot_stride = batch * 1024;
    diag128_potrf<<<dim3(batch), dim3(D128_THREADS), D128_SHARED_BYTES>>>(
        src.data_ptr<float>(), work.data_ptr<float>(),
        inv.data_ptr<float>(), (int)n, (int)j, (int)first, slot_stride);
    cudaError_t status = cudaGetLastError();
    TORCH_CHECK(status == cudaSuccess, cudaGetErrorString(status));
}
...
def _load_cuda256() -> None:
    """Experiment 050: every CUDA kernel now lives in one extension, built
    eagerly below, so this is a no-op kept for call-site compatibility."""
    return


# ---------------------------------------------------------------------------
# Experiment 050: ONE extension for every CUDA kernel.
#
# The official runner keeps a build cache keyed by extension name. The ranked
# source's test passes in 91s because all of its extensions are cache hits; a
# probe that changed nothing but the extension name failed at exactly the 360s
# service timeout (submission #898675). So the real budget is the *cold* build,
# and four `load_inline` calls means compiling the very expensive
# `torch/extension.h` pybind glue four times. Merging them into one module
# pays that cost once and is what makes any new CUDA kernel shippable at all.
#
    ...
    try:
        ...
        _CUDA_ALL = load_inline(
            ...
            cpp_sources=(
                ...
                "void diag128_launch(torch::Tensor, torch::Tensor, "
                "torch::Tensor, int64_t, int64_t, int64_t);"
            ),
...
# Experiment 050: shapes whose whole 128-wide diagonal block is factored by the
# single `diag128_potrf` CUDA CTA instead of the seven-launch Triton chain.
#
# The kernel cannot be captured into a CUDA graph (a raw `<<<grid, block>>>`
# launch has no capturable work queue and popcorn's source policy forbids
# naming one), so every enrolled shape must also run in `eager` mode. That is
# only a win because the fusion removes far more launches than eager mode
# costs: at 16x512 the shipped graph replays 53 launches, and the fused
# schedule emits 11.
#
# Maps shape -> (panel TILE_R, panel num_warps).
_DIAG128_SHAPES = {
    (16, 512): (128, 8),
    (4, 1024): (128, 8),
    # exp 050: 2x2048 was still 91% cuSOLVER `getrf_wo_pivot`, which factors
    # the two matrices serially at 611.8us each. The n=2048 split32 settings
    # are the ones 8x2048 already ships for every input family.
    (2, 2048): (128, 8),
}
_DIAG128_HITS = 0
...
if _HAVE_TRITON:
    ...
    _SPLIT32_SHAPES = {
        ...
        (16, 512): ("tf32x3", "tf32x3", 128, "eager", True),
    ...
    def _split32_launch(
        ...
        global _FUSED_PANEL_HITS, _DIAG128_HITS
        batch, n, _ = work.shape
        diag128_cfg = _DIAG128_SHAPES.get((batch, n))
        if diag128_cfg is not None and _CUDA128 is not None:
            ftile, fwarps = diag128_cfg
            ft = src is not None
            if not ft:
                src = work
            for j in range(0, n, 128):
                _DIAG128_HITS += 1
                _CUDA128.diag128_launch(
                    src, work, dinv, n, j, 1 if (ft and j == 0) else 0
                )
                rem_out = n - j - 128
                if rem_out <= 0:
                    break
    ...
    def _dinv_slots(batch, n):
        ...
        if (batch, n) in _FUSED_PANEL_SHAPES or (batch, n) in _DIAG128_SHAPES:
            return 4
        return 1
