...
// Staging stride for the finished 32x32 factor. 36 is a multiple of four, so
// every row starts 16-byte aligned and the triangular-inverse reduction can
// read it with `float4` loads; the main tile keeps the odd 129 stride because
// its hot accesses are column-strided.
constexpr int D128_SUB_LDS = 36;
constexpr int D128_SHARED_BYTES =
    (D128_N * D128_LDS + 32 * D128_INV_LDS + 4 * 32 + 32 * D128_SUB_LDS)
    * (int)sizeof(float);
...
void diag128_potrf(const float* __restrict__ src, float* __restrict__ work,
    ...
    for (int kb = 0; kb < D128_N; kb += 32) {
        if (warp == 0) {
            float* pivot_row = tile + (size_t)(kb + lane) * D128_LDS + kb;
            float values[32];
            #pragma unroll
            for (int c = 0; c < 32; ++c) values[c] = pivot_row[c];
            ...
            for (int iteration = 0; iteration < 8; ++iteration) {
                ...
                const float rec0 =
                    rsqrtf(__shfl_sync(MICRO_FULL, values[p0], p0));
                const float s0 = (lane >= p0) ? values[p0] * rec0 : 0.0f;
                values[p0] = s0;
                pivots[0 * 32 + lane] = s0;
                const float c01 = __shfl_sync(MICRO_FULL, s0, p1);
                ...
                const float rec1 =
                    rsqrtf(__shfl_sync(MICRO_FULL, values[p1], p1));
                const float s1 = (lane >= p1) ? values[p1] * rec1 : 0.0f;
                values[p1] = s1;
                pivots[1 * 32 + lane] = s1;
                const float c12 = __shfl_sync(MICRO_FULL, s1, p2);
                ...
                const float rec2 =
                    rsqrtf(__shfl_sync(MICRO_FULL, values[p2], p2));
                const float s2 = (lane >= p2) ? values[p2] * rec2 : 0.0f;
                values[p2] = s2;
                pivots[2 * 32 + lane] = s2;
                const float c23 = __shfl_sync(MICRO_FULL, s2, p3);
                ...
                const float rec3 =
                    rsqrtf(__shfl_sync(MICRO_FULL, values[p3], p3));
                const float s3 = (lane >= p3) ? values[p3] * rec3 : 0.0f;
                values[p3] = s3;
                pivots[3 * 32 + lane] = s3;
                ...
                // Every lane reads the same four pivot rows, so the whole
                // rank-4 trailing update is one broadcast sequence. Reading it
                // four values at a time turns 448 dependent `LDS` issues per
                // 32x32 block into 112 `LDS.128`s, which is what lets a lone
                // warp keep its FMA pipe fed: the scalar form exposed a
                // shared-load latency on nearly every FMA.
                #pragma unroll
                for (int c = p3 + 1; c < 32; c += 4) {
                    const float4 q0 =
                        *reinterpret_cast<const float4*>(pivots + 0 * 32 + c);
                    const float4 q1 =
                        *reinterpret_cast<const float4*>(pivots + 1 * 32 + c);
                    const float4 q2 =
                        *reinterpret_cast<const float4*>(pivots + 2 * 32 + c);
                    const float4 q3 =
                        *reinterpret_cast<const float4*>(pivots + 3 * 32 + c);
                    if (c + 0 <= lane) {
                        float value = values[c + 0];
                        value = fmaf(-s0, q0.x, value);
                        value = fmaf(-s1, q1.x, value);
                        value = fmaf(-s2, q2.x, value);
                        value = fmaf(-s3, q3.x, value);
                        values[c + 0] = value;
                    }
                    if (c + 1 <= lane) {
                        float value = values[c + 1];
                        value = fmaf(-s0, q0.y, value);
                        value = fmaf(-s1, q1.y, value);
                        value = fmaf(-s2, q2.y, value);
                        value = fmaf(-s3, q3.y, value);
                        values[c + 1] = value;
                    }
                    if (c + 2 <= lane) {
                        float value = values[c + 2];
                        value = fmaf(-s0, q0.z, value);
                        value = fmaf(-s1, q1.z, value);
                        value = fmaf(-s2, q2.z, value);
                        value = fmaf(-s3, q3.z, value);
                        values[c + 2] = value;
                    }
                    if (c + 3 <= lane) {
                        float value = values[c + 3];
                        value = fmaf(-s0, q0.w, value);
                        value = fmaf(-s1, q1.w, value);
                        value = fmaf(-s2, q2.w, value);
                        value = fmaf(-s3, q3.w, value);
                        values[c + 3] = value;
                    }
                }
                __syncwarp();
            ...
            float* sub_row = sub_factor + lane * D128_SUB_LDS;
            #pragma unroll
            for (int c = 0; c < 32; ++c) {
            ...
            pivots[0 * 32 + lane] = reciprocal_row;
            __syncwarp();

            // Triangular inverse of the finished 32x32 factor: lane owns
            // column `lane` of X, row r is final after step r. The reduction
            // reads the factor row broadcast, so it vectorises the same way.
            float inverse[32];
            #pragma unroll
            for (int r = 0; r < 32; ++r) {
                const float4* factor_row = reinterpret_cast<const float4*>(
                    sub_factor + r * D128_SUB_LDS);
                float accumulator = (r == lane) ? 1.0f : 0.0f;
                #pragma unroll
                for (int q = 0; 4 * q < r; ++q) {
                    const float4 f = factor_row[q];
                    if (4 * q + 0 < r) {
                        accumulator =
                            fmaf(-f.x, inverse[4 * q + 0], accumulator);
                    }
                    if (4 * q + 1 < r) {
                        accumulator =
                            fmaf(-f.y, inverse[4 * q + 1], accumulator);
                    }
                    if (4 * q + 2 < r) {
                        accumulator =
                            fmaf(-f.z, inverse[4 * q + 2], accumulator);
                    }
                    if (4 * q + 3 < r) {
                        accumulator =
                            fmaf(-f.w, inverse[4 * q + 3], accumulator);
                    }
                }
            ...
            #pragma unroll
            for (int r = 0; r < 32; ++r) {
                dinv_s[r * D128_INV_LDS + lane] = inverse[r];
                inverse_out[r * 32] = inverse[r];
            }
        }
        __syncthreads();
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA128 = load_inline(
            name="chol128_exp042_v5_exp044_micro_exp050_diag",
            cpp_sources=(
            ...
            functions=["chol128_launch", "micro32_launch", "diag128_launch"],
            extra_cuda_cflags=["-O3"],
            verbose=False,
        )
    except Exception as exc:
...
if _HAVE_TRITON:
    ...
    _SPLIT32_SHAPES = {
        ...
        (8, 2048): ("tf32", "tf32", 128, "eager", True),
        # exp 050: these three were still on cuSOLVER, which factors each
        # matrix serially -- 611.8us per 2048 and 1387.3us per 4096 matrix,
        # 87-91% of the whole shape. The fused diagonal block makes the
        # split32 route cheaper than the vendor path at batch 1-2.
        (2, 2048): ("tf32", "tf32", 128, "eager", True),
        (2, 4096): ("tf32", "tf32", 128, "eager", True),
    }
    _SPLIT32_TILE = 128
    _SPLIT32_NB = 128
