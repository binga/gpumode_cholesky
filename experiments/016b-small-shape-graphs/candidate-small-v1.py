...
"""GPU MODE `cholesky` submission — experiment 016b candidate v1 (small
shapes: graphed Triton 4096x32, split32 at 1024x64 and 256x128).
...
if _HAVE_TRITON:
    ...
    _SPLIT32_SHAPES = {
        (1024, 64): ("tf32x3", "tf32x3", 128),
        (256, 128): ("tf32x3", "tf32x3", 128),
...
def _graph_triton_4096x32(data):
    """Graph-replayed Triton n=32 factorization for the ranked (4096, 32)
    shape: removes the per-call contiguous/empty_like/launch overhead."""
    ...
    if state is None:
        try:
            ...
            for _ in range(2):
                _triton_cholesky32(static_in)
            torch.cuda.synchronize()
            graph = torch.cuda.CUDAGraph()
            with torch.cuda.graph(graph, pool=_shared_graph_pool()):
                static_out = _triton_cholesky32(static_in)
            graph.replay()
            torch.cuda.synchronize()
