...
_SHADOW_FP8_32768_HITS = 0
_SHADOW_FP8_32768_UPDATE_HITS = 0
_SHADOW_FP8_32768_QUANT_HITS = 0
_SHADOW_FP8_32768_ERROR = None
...
def _left_looking_cholesky_32768(mat: torch.Tensor) -> torch.Tensor:
    global _LEFT_32768_HITS, _SHADOW_FP8_32768_HITS
    global _SHADOW_FP8_32768_UPDATE_HITS, _SHADOW_FP8_32768_QUANT_HITS

    nb = 4096
    n = mat.shape[0]
    factor = torch.zeros_like(mat)
    quantized_factor = torch.empty_like(factor, dtype=torch.float8_e4m3fn)
    encode_scale = 128.0
    decode_scale = torch.full(
        (), 1.0 / encode_scale, device=mat.device, dtype=torch.float32
    )
    previous_tf32 = torch.backends.cuda.matmul.allow_tf32
    try:
        torch.backends.cuda.matmul.allow_tf32 = True
        for k in range(0, n, nb):
            ...
            if k:
                quantized_lhs = quantized_factor[j:, :k].contiguous()
                quantized_rhs = (
                    quantized_factor[k : k + kb, :k]
                    .transpose(-1, -2)
                    .contiguous()
                )
                panel.sub_(
                    _scaled_mm_fp8_32768(
                        quantized_lhs,
                        quantized_rhs,
                        decode_scale,
                        decode_scale,
                ...
                _SHADOW_FP8_32768_UPDATE_HITS += 1
            identity = torch.eye(
            ...
            completed_column = factor[k:, k : k + kb]
            quantized_factor[k:, k : k + kb] = (
                completed_column * encode_scale
            ).to(torch.float8_e4m3fn)
            _SHADOW_FP8_32768_QUANT_HITS += 1
    finally:
        torch.backends.cuda.matmul.allow_tf32 = previous_tf32
    _LEFT_32768_HITS += 1
    _SHADOW_FP8_32768_HITS += 1
    return factor
