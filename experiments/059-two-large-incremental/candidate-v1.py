...
# ---------------------------------------------------------------------------
# Experiment 059: two-large-shape blocked triangular inverse.
#
# The incumbent recursive inverse bottoms out in one dense-identity triangular
# solve per leaf and emits elementwise scaffolding at every internal node.
# Instead, invert all base-sized diagonal leaves in one batched triangular solve
# and combine each tree level with two batched GEMMs.  The panel apply then uses
# a tensor-core GEMM.  The arithmetic mode and block widths remain identical to
# ranked #890798; only batch=1,n in {16384,32768} is enrolled.
# ---------------------------------------------------------------------------
...
def _blocked_tri_inv(lower: torch.Tensor, base: int = 256) -> torch.Tensor:
    """Breadth-first inverse of a power-of-two lower-triangular block."""
    n = lower.shape[0]
...
def _large_v2(
    ...
    try:
        ...
        for k in range(0, n, nb):
            ...
            j = k + kb
            if update == "mxfp8":
                # Preserve the incumbent 32768 arithmetic: TF32 diagonal
                # update and MXFP8 below-diagonal panel update.
                ...
                # Update the diagonal block and panel below it in one GEMM.
