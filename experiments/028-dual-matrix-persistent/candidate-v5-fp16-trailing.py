...
if _HAVE_TRITON:
    ...
    # ------------------------------------------------------------------
    # Experiment 028: persistent, cuSOLVER-free factorization for the two
    # batch-2 ranked shapes. A deliberately small resident grid executes every
    # phase inside one Triton launch. Atomic phase counters are device-wide
    # barriers; 32 total CTAs is safely below the B200 SM count even at one CTA
    # per SM, avoiding the forward-progress hazard of an over-subscribed grid.
    # ------------------------------------------------------------------
    # Variant 2: occupy 128 of B200's 148 SMs so the rank-128 trailing phase
    # executes in roughly two resident waves instead of up to eight.
    _DUAL_PERSIST_WORKERS = 64
    _DUAL_PERSIST_TOTAL = 128
    _DUAL_PERSIST_STATE = {}
    _DUAL_PERSIST_HITS = 0
    _DUAL_PERSIST_FALLBACKS = 0
    _DUAL_PERSIST_ERROR = None

    @triton.jit
    def _dual_phase_barrier(state_ptr, phase, TOTAL: tl.constexpr):
        tl.atomic_add(state_ptr + phase, 1)
        observed = tl.atomic_cas(state_ptr + phase, TOTAL, TOTAL)
        while observed != TOTAL:
            observed = tl.atomic_cas(state_ptr + phase, TOTAL, TOTAL)

    @triton.jit
    def _dual_persistent_cholesky(
        out_ptr,
        inv_ptr,
        state_ptr,
        n: tl.constexpr,
        ...
        """Resident two-matrix blocked Cholesky.

        One worker per matrix advances the dependent 32x32 diagonal chain.
        All workers apply panels and trailing updates. The two matrices share
        phase barriers but otherwise use disjoint storage, so their independent
        chains are exposed together without host launches between phases.
        """
        pid = tl.program_id(0)
        b = pid // WORKERS
        wid = pid - b * WORKERS
        base = b.to(tl.int64) * n * n
        phase = 0
        j = 0
        while j < n:
            micro = 0
            while micro < 4:
                ...
                # Dependent diagonal microblock and its triangular inverse.
                if wid == 0:
                    r = tl.arange(0, 32)
                    c = tl.arange(0, 32)
                    off = (k + r)[:, None] * n + (k + c)[None, :]
                    ptr = out_ptr + base + off
                    a = tl.load(ptr)
                    x = tl.where(r[:, None] == c[None, :], 1.0, 0.0)
                    for it in range(0, 16):
                        ...
                        q = p + 1
                        colp = tl.sum(
                            tl.where(c[None, :] == p, a, 0.0), axis=1
                        )
                        colq = tl.sum(
                            tl.where(c[None, :] == q, a, 0.0), axis=1
                        )
                        dpp = tl.sum(tl.where(r == p, colp, 0.0), axis=0)
                        aqq = tl.sum(tl.where(r == q, colq, 0.0), axis=0)
                        invp = 1.0 / tl.sqrt(dpp)
                        lp = tl.where(r >= p, colp * invp, 0.0)
                        lqp = tl.sum(tl.where(r == q, lp, 0.0), axis=0)
                        dqq = aqq - lqp * lqp
                        invq = 1.0 / tl.sqrt(dqq)
                        lq = tl.where(
                            r >= q, (colq - lqp * lp) * invq, 0.0
                        )
                        trail = (r[:, None] > q) & (c[None, :] > q)
                        a = tl.where(
                            c[None, :] == p,
                        ...
                        rowp = tl.sum(
                            tl.where(r[:, None] == p, a, 0.0), axis=0
                        )
                        rowq = tl.sum(
                            tl.where(r[:, None] == q, a, 0.0), axis=0
                        )
                        gp = tl.sum(
                            tl.where(c < p, rowp, 0.0)[:, None] * x,
                            axis=0,
                        )
                        gq = tl.sum(
                            tl.where(c < p, rowq, 0.0)[:, None] * x,
                            axis=0,
                        )
                        ep = tl.where(c == p, 1.0, 0.0)
                        eq = tl.where(c == q, 1.0, 0.0)
                        xp = (ep - gp) * invp
                        xq = (eq - gq - lqp * xp) * invq
                        x = tl.where(
                            r[:, None] == p,
                            xp[None, :],
                    ...
                    tl.store(ptr, a)
                    tl.store(
                        inv_ptr + b.to(tl.int64) * 1024
                        + r[:, None] * 32
                        + c[None, :],
                        x,
                    )

                    # The diagonal worker also prepares and updates the rows
                    # inside this 128-column outer panel. These rows are the
                    # right operand needed by every lower-row worker, so doing
                    # this before the first phase barrier lets those workers
                    # fuse their solve and panel update without another global
                    # rendezvous.
                    width_head = j + 128 - (k + 32)
                    if width_head > 0:
                        head_rows = tl.arange(0, 128)
                        d32 = tl.arange(0, 32)
                        hmask = head_rows < width_head
                        h_ptrs = (
                            out_ptr
                            + base
                            + (k + 32 + head_rows)[:, None] * n
                            + (k + d32)[None, :]
                        )
                        head = tl.load(h_ptrs, mask=hmask[:, None], other=0.0)
                        dinv_head = tl.load(
                            inv_ptr + b.to(tl.int64) * 1024
                            + d32[:, None] * 32
                            + d32[None, :]
                        ...
                        tl.store(h_ptrs, lhead, mask=hmask[:, None])
                        hmirror = (
                            out_ptr
                            + base
                            + (k + d32)[:, None] * n
                        ...
                        tl.store(
                            hmirror,
                            tl.zeros((32, 128), dtype=tl.float32),
                            mask=hmask[None, :],
                        )
                        tl.debug_barrier()
                        head_tiles = (width_head + 63) // 64
                        ht = 0
                        while ht < head_tiles:
                            hr = ht * 64 + tl.arange(0, 64)
                            hrmask = hr < width_head
                            li_head = tl.load(
                                out_ptr
                                + base
                                + (k + 32 + hr)[:, None] * n
                            ...
                            hc = 0
                            while hc < head_tiles:
                                hcols = hc * 64 + tl.arange(0, 64)
                                hcmask = hcols < width_head
                                lj_head = tl.load(
                                    out_ptr
                                    + base
                                    + (k + 32 + hcols)[:, None] * n
                                ...
                                hvalid = hrmask[:, None] & hcmask[None, :]
                                htile = tl.load(
                                    ht_ptrs, mask=hvalid, other=0.0
                                )
                                tl.store(
                                    ht_ptrs, htile - hprod, mask=hvalid
                                )
                                hc += 1
                            ht += 1

                _dual_phase_barrier(state_ptr, phase, TOTAL)
                phase += 1

                # Each worker solves its rows below the outer panel and then
                # immediately applies their rank-32 contribution to the rest
                # of the panel. The solved tile stays in registers.
                below = n - j - 128
                width = j + 128 - (k + 32)
                task = wid
                row_tasks = (below + 63) // 64
                while task < row_tasks:
                    rows = task * 64 + tl.arange(0, 64)
                    c32 = tl.arange(0, 32)
                    rmask = rows < below
                    p_ptrs = (
                        out_ptr
                    ...
                    panel = tl.load(
                        p_ptrs, mask=rmask[:, None], other=0.0
                    )
                    dinv = tl.load(
                        inv_ptr + b.to(tl.int64) * 1024
                        + c32[:, None] * 32
                    ...
                    tl.store(p_ptrs, lik, mask=rmask[:, None])
                    mirror = (
                        out_ptr
                        + base
                        + (k + c32)[:, None] * n
                    ...
                    tl.store(
                        mirror,
                        tl.zeros((32, 64), dtype=tl.float32),
                        mask=rmask[None, :],
                    )
                    if width > 0:
                        col_tiles = (width + 63) // 64
                        ct = 0
                        while ct < col_tiles:
                            cols = ct * 64 + tl.arange(0, 64)
                            cmask = cols < width
                            lj = tl.load(
                                out_ptr
                                + base
                            ...
                            tile = tl.load(t_ptrs, mask=valid, other=0.0)
                            tl.store(t_ptrs, tile - prod, mask=valid)
                            ct += 1
                    task += WORKERS
            ...
            # Rank-128 lower-triangular Schur update for the completed panel.
            rem_out = n - j - 128
            if rem_out > 0:
                tile_count = (rem_out + 127) // 128
                trailing_tasks = tile_count * (tile_count + 1) // 2
                task = wid
                while task < trailing_tasks:
                    br = ((tl.sqrt(8.0 * task + 1.0) - 1.0) * 0.5).to(
                        tl.int32
                    )
                    bc = task - br * (br + 1) // 2
                    rows = br * 128 + tl.arange(0, 128)
                    cols = bc * 128 + tl.arange(0, 128)
                    d = tl.arange(0, 128)
                    rmask = rows < rem_out
                    cmask = cols < rem_out
                    li = tl.load(
    ...
    def _dual_persistent_factor(data: torch.Tensor) -> torch.Tensor:
        global _DUAL_PERSIST_HITS
        batch, n, _ = data.shape
        out = data.contiguous().clone()
        key = (n, data.device)
        cached = _DUAL_PERSIST_STATE.get(key)
        if cached is None:
            state = torch.empty(512, dtype=torch.int32, device=data.device)
            inv = torch.empty(batch, 32, 32, dtype=torch.float32, device=data.device)
            cached = (state, inv)
            _DUAL_PERSIST_STATE[key] = cached
        state, inv = cached
        state.zero_()
        _dual_persistent_cholesky[(_DUAL_PERSIST_TOTAL,)](
            out,
            inv,
            state,
            n=n,
            WORKERS=_DUAL_PERSIST_WORKERS,
            TOTAL=_DUAL_PERSIST_TOTAL,
            num_warps=8,
            num_stages=3,
        )
        _DUAL_PERSIST_HITS += 1
        return out
...
def custom_kernel(data: input_t) -> output_t:
    ...
    global _DUAL_PERSIST_FALLBACKS, _DUAL_PERSIST_ERROR
    ...
    if is_f32_cuda and _HAVE_TRITON and batch == 2 and n in (2048, 4096):
        try:
            l = _dual_persistent_factor(data)
            if torch.isfinite(l.diagonal(dim1=-2, dim2=-1)).all().item():
                _DUAL_PERSIST_ERROR = None
                return l
            _DUAL_PERSIST_FALLBACKS += 1
        except Exception as exc:
            _DUAL_PERSIST_ERROR = repr(exc)
            _DUAL_PERSIST_FALLBACKS += 1
