...
if _HAVE_TRITON:
    ...
    def _dual_persistent_cholesky(
        ...
        while j < n:
            ...
            while micro < 2:
                ...
                # Apply the inverse to all rows below the microblock.
                remaining = n - k - 64
                task = wid
                row_tasks = (remaining + 127) // 128
                while task < row_tasks:
                    rows = task * 128 + tl.arange(0, 128)
                    c64 = tl.arange(0, 64)
                    rmask = rows < remaining
                    p_ptrs = (
                        out_ptr
                    ...
                    tl.store(
                        mirror,
                        tl.zeros((64, 128), dtype=tl.float32),
                        mask=rmask[None, :],
                    )
                    task += WORKERS
                ...
                # Update only the remainder of this 128-column panel.
                width = j + 128 - (k + 64)
                if width > 0:
                    row_tiles = (remaining + 63) // 64
                    col_tiles = (width + 63) // 64
                    task = wid
                    panel_tasks = row_tiles * col_tiles
                    while task < panel_tasks:
                        rt = task // col_tiles
                        ct = task - rt * col_tiles
                        rows = rt * 64 + tl.arange(0, 64)
    ...
    def _dual_persistent_factor(data: torch.Tensor) -> torch.Tensor:
        ...
        if cached is None:
            ...
            inv = torch.empty(batch, 64, 64, dtype=torch.float32, device=data.device)
            cached = (state, inv)
