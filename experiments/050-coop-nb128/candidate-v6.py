"""Experiment 050 V6: single cooperative NB=128 tensor-core Cholesky, 4x1024.
...
// tiles at 69632 B; Phase P keeps xs/ys/xt strips (3 x 16896), the tf32 E
// factor (67584) and the four staged inverses (18432) resident = 136704 B.
...
__global__ void coop_cholesky(float* mats, float* dinvg, long long* prof) {
    ...
    for (int kt = 0; kt < N / NB; ++kt) {
        ...
        if (r > 0) {
            ...
            // L11 = D + E with D the four diagonal 32-blocks and E strictly
            // block-lower. X = (A - X E^T) D^-T iterated four times IS block
            // forward substitution (E^T D^-T is nilpotent of degree 4), but
            // each iteration is one full-width 32x128x128 tf32 WMMA product:
            // no per-column-block staging chain. Identical rounding
            // structure to the substitution it replaces (operands RN-tf32,
            // fp32 accumulate).
            {
                const int S = r / MT;
                ...
                float (*le)[LDS] =
                    reinterpret_cast<float (*)[LDS]>(smem + 3 * MT * LDS);
                float (*dv)[144] = reinterpret_cast<float (*)[144]>(
                    smem + 3 * MT * LDS + NB * LDS);
                const int wr = warp & 1;
                const int wc = warp >> 1;
                    ...
                    for (int idx = tid; idx < NB * NB; idx += THREADS) {
                        ...
                        le[rw][cw] = ((rw >> 5) > (cw >> 5))
                            ? wmma::__float_to_tf32(
                                  M[(size_t)(k0 + rw) * N + k0 + cw])
                            : 0.0f;
                    }
                    for (int idx = tid; idx < MT * NB; idx += THREADS) {
                        const int rw = idx / NB;
                        const int b = (idx % NB) >> 5;
                        const int cc = idx & 31;
                        dv[rw][36 * b + cc] = wmma::__float_to_tf32(
                            dinvg[((size_t)m * SUB + b) * (MT * MT)
                                  + (size_t)rw * MT + cc]);
                    }
                    __syncthreads();
                    #pragma unroll
                    for (int itn = 0; itn < SUB; ++itn) {
                        // T = A - Y_i E^T (fp32 accumulate, tf32 product).
                        if (itn == 0) {
                        ...
                        __syncthreads();
                        // Y_{i+1} = T D^-T: column-block wc solves against
                        // its own staged 32x32 inverse.
                        {
                            wmma::fragment<wmma::accumulator, 16, 16, 8,
                                           float> sol0, sol1;
                            ...
                            for (int kk = 0; kk < MT; kk += 8) {
                                ...
                                wmma::load_matrix_sync(
                                    bf0, &dv[0][36 * wc + kk], 144);
                                wmma::load_matrix_sync(
                                    bf1, &dv[16][36 * wc + kk], 144);
