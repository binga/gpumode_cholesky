...
# Loaded together with CUDA64 and CUDA128 below. A single combined extension
# removes two fixed NVCC/PyBind startup costs while compiling the exact kernels.
...
# Loaded together with CUDA32 and CUDA128 below.
...
// --- Experiment 050: single-launch cooperative NB=128 Cholesky for 4x1024
// (replaces the 101-launch dependent chain). Compiled into this module to
// keep the nvcc invocation count unchanged (six-minute official compile
// budget, see exp 044).
...
__device__ __forceinline__ void _subtrail32(float (*ds)[NB + 5], int o2,
                                            int o, int tid) {
    // Register-blocked: each thread owns a 2-row x 4-col output tile, so
    // every shared load is amortized over 4 (row operand) or 2 (column
    // operand) FMAs. Upper-triangle tiles are skipped whole.
    constexpr int TR = RR / 2;   // tile rows index space
    constexpr int TC = RR / 4;   // tile cols index space
...
__global__ void coop_cholesky(float* mats, float* dinvg) {
...
void coop1024_launch(torch::Tensor mats, torch::Tensor scratch) {
    coop50::launch_coop<1024, 4>(mats, scratch);
}
...
if torch.cuda.is_available():
    try:
        ...
        _CUDA128 = load_inline(
            name="chol3264128_exp050_v16_combined_o3",
            cpp_sources=(
                ...
                "void coop1024_launch(torch::Tensor, torch::Tensor);"
            ),
            cuda_sources=(
                _CUDA32_SOURCE + "\n" + _CUDA64_SOURCE + "\n" +
                _CUDA128_SOURCE
            ...
            functions=["chol32_launch", "chol64_launch", "chol128_launch",
                       "micro32_launch", "coop1024_launch"],
...
# Experiment 050: single cooperative-launch path for 4x1024.
_COOP1024_HITS = 0
...
def _coop1024_scratch(mats):
    key = mats.device.index
    buf = _COOP1024_SCRATCH.get(key)
    if buf is None:
        buf = torch.empty(
            4 * 4 * 32 * 32, device=mats.device, dtype=torch.float32
        )
        _COOP1024_SCRATCH[key] = buf
    return buf
...
def custom_kernel(data: input_t) -> output_t:
    ...
    global _COOP1024_HITS, _COOP1024_FALLBACKS, _COOP1024_ERROR
    ...
    if (
        ...
        # Experiment 050: one cooperative launch replaces the dependent
        # chain. Finite-diagonal retry contract: any non-finite pivot falls
        # through to the previously ranked dispatch below.
        try:
            l = data.clone()
            _CUDA128.coop1024_launch(l, _coop1024_scratch(l))
