"""Experiment 050 V2: single cooperative NB=128 tensor-core Cholesky, 4x1024.
        ...
        if (blockIdx.x < MATS) {
            ...
            for (int s = 0; s < SUB; ++s) {
                ...
                if (warp == 0) {
                    ...
                    __syncwarp();
                    // Exact fp32 inverse; lane owns column `lane` of L^-1,
                    // divides replaced by the cached reciprocals.
                    float inverse[32];
                    #pragma unroll
                    for (int r = 0; r < 32; ++r) {
                        ...
                        for (int p = 0; p < 32; ++p) {
                            if (p < r)
                                acc = fmaf(-ds[o + r][o + p], inverse[p],
                                           acc);
                        }
                        inverse[r] =
                            (r >= lane) ? acc * pivots[0][r] : 0.0f;
                        dinv_sh[r][lane] = inverse[r];
                    }
                }
                __syncthreads();
                if (prof_lead) {
                ...
                if (rr > 0) {
                    ...
                    // Static-indexed staging keeps tmp in registers.
                    constexpr int PITEMS = (96 * 32 + THREADS - 1) / THREADS;
                    float tmp[PITEMS];
                    #pragma unroll
                    for (int t = 0; t < PITEMS; ++t) {
                        ...
                        if (idx < rr * MT) {
                            ...
                            float v = 0.0f;
                            for (int p = 0; p <= c; ++p)
                                v = fmaf(ds[o2 + r][o + p], dinv_sh[c][p],
                                         v);
                            tmp[t] = v;
                        }
                    }
