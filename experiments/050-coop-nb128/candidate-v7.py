"""Experiment 050 V7: single cooperative NB=128 tensor-core Cholesky, 4x1024.
...
// Phase D needs 128*133 fp32 = 68096 B; Phase P keeps xs/ys/xt strips
// resident (3 x 16896 = 50688 B); Phase T reads fp16 operands straight from
// the global panel copy and stages nothing. Small smem keeps the L1
// carveout large for the global-operand tensor-core loads.
constexpr int SMEM_BYTES = NB * LDD * 4;         // 68096
...
__global__ void coop_cholesky(float* mats, float* scratch, __half* panelh,
                              long long* prof) {
    // scratch layout per matrix: 4x32x32 fp32 inverses (RN-tf32 values),
    // then the 128x128 strictly-block-lower tf32 E factor.
    float* dinvg = scratch;
    float* leg = scratch + (size_t)MATS * SUB * MT * MT;
    ...
    for (int kt = 0; kt < N / NB; ++kt) {
        ...
        if (blockIdx.x < MATS) {
            ...
            for (int idx = tid; idx < NB * NB; idx += THREADS) {
                ...
                leg[(size_t)m * NB * NB + idx] = ((rw >> 5) > (cw >> 5))
                    ? wmma::__float_to_tf32(ds[rw][cw]) : 0.0f;
            }
            __syncthreads();
        ...
        const int r = N - k0 - NB;
        if (r > 0) {
            // ---- Phase P: panel TRSM, Neumann/nilpotent form -----------
            // L11 = D + E, X = (A - X E^T) D^-T iterated SUB times is exact
            // block forward substitution (E^T D^-T nilpotent). Tensor-core
            // operands (E factor, 32x32 inverses) are read directly from the
            // RN-tf32 global copies published by Phase D -- no shared
            // staging. Each strip also writes an fp16 copy of its solved
            // rows for the zero-staging trailing phase.
            {
                const int S = r / MT;
                ...
                float (*xt)[LDS] =
                    reinterpret_cast<float (*)[LDS]>(smem + 2 * MT * LDS);
                const int wr = warp & 1;
                const int wc = warp >> 1;
                for (int item = blockIdx.x; item < total;
                    ...
                    float* M = mats + (size_t)m * N * N;
                    const float* le = leg + (size_t)m * NB * NB;
                    const float* dvg =
                        dinvg + (size_t)m * SUB * MT * MT;
                    __syncthreads();
                    ...
                    __syncthreads();
                    #pragma unroll
                    for (int itn = 0; itn < SUB; ++itn) {
                        if (itn == 0) {
                            for (int idx = tid; idx < MT * NB;
                                 idx += THREADS)
                                xt[idx / NB][idx % NB] =
                            ...
                            for (int kk = 0; kk < NB; kk += 8) {
                                ...
                                wmma::load_matrix_sync(
                                    af, &xt[wr * 16][kk], LDS);
                                wmma::load_matrix_sync(
                                    bf0, le + (size_t)(wc * 32) * NB + kk,
                                    NB);
                                wmma::load_matrix_sync(
                                    bf1,
                                    le + (size_t)(wc * 32 + 16) * NB + kk,
                                    NB);
                            ...
                            wmma::store_matrix_sync(
                                &ys[wr * 16][wc * 32], acc0, LDS,
                                wmma::mem_row_major);
                            wmma::store_matrix_sync(
                                &ys[wr * 16][wc * 32 + 16], acc1, LDS,
                                wmma::mem_row_major);
                            __syncthreads();
                            for (int idx = tid; idx < MT * NB;
                                 idx += THREADS) {
                                const int rw = idx / NB, cw = idx % NB;
                                xt[rw][cw] = wmma::__float_to_tf32(
                                    xs[rw][cw] - ys[rw][cw]);
                            }
                        }
                        __syncthreads();
                        {
                            wmma::fragment<wmma::accumulator, 16, 16, 8,
                                           float> sol0, sol1;
                            wmma::fill_fragment(sol0, 0.0f);
                            wmma::fill_fragment(sol1, 0.0f);
                            #pragma unroll
                            for (int kk = 0; kk < MT; kk += 8) {
                                ...
                                wmma::fragment<wmma::matrix_b, 16, 16, 8,
                                               ...
                                               wmma::col_major> bf0, bf1;
                                wmma::load_matrix_sync(
                                    af, &xt[wr * 16][wc * 32 + kk], LDS);
                                wmma::load_matrix_sync(
                                    bf0,
                                    dvg + (size_t)wc * MT * MT + kk, MT);
                                wmma::load_matrix_sync(
                                    bf1,
                                    dvg + (size_t)wc * MT * MT + 16 * MT
                                        + kk,
                                    MT);
                                wmma::mma_sync(sol0, af, bf0, sol0);
                                wmma::mma_sync(sol1, af, bf1, sol1);
                            }
                            __syncthreads();
                            wmma::store_matrix_sync(
                                &ys[wr * 16][wc * 32], sol0, LDS,
                                wmma::mem_row_major);
                            wmma::store_matrix_sync(
                                &ys[wr * 16][wc * 32 + 16], sol1, LDS,
                                wmma::mem_row_major);
                            __syncthreads();
                        }
                        if (itn + 1 < SUB) {
                    ...
                    __half* ph = panelh
                        + (size_t)m * N * NB + (size_t)(row0 - k0 - NB) * NB;
                    for (int idx = tid; idx < MT * NB / 4; idx += THREADS) {
                        const int rw = idx / (NB / 4);
                        const int c4 = 4 * (idx % (NB / 4));
                        const float4 v = make_float4(
                            ys[rw][c4], ys[rw][c4 + 1],
                            ys[rw][c4 + 2], ys[rw][c4 + 3]);
                        *reinterpret_cast<float4*>(
                            &M[(size_t)(row0 + rw) * N + k0 + c4]) = v;
                        __half2* ph2 = reinterpret_cast<__half2*>(
                            ph + (size_t)rw * NB + c4);
                        ph2[0] = __floats2half2_rn(v.x, v.y);
                        ph2[1] = __floats2half2_rn(v.z, v.w);
                    }
                }
            }
            grid.sync();
            ...
            // Operands come straight from the RN-fp16 panel copy written by
            // Phase P (global, L2-hot): no shared staging, no __syncthreads
            // in the tile loop. fp16 products with fp32 accumulation is the
            // shipped FP16_TRAILING structure at this shape.
            {
                const int t128 = r / NB;
                ...
                for (long item = blockIdx.x; item < total;
                    ...
                    float* M = mats + (size_t)m * N * N;
                    const __half* pa =
                        panelh + (size_t)m * N * NB + (size_t)prow * NB * NB;
                    const __half* pb =
                        panelh + (size_t)m * N * NB + (size_t)pcol * NB * NB;
                    ...
                    for (int kk = 0; kk < NB; kk += 16) {
                        wmma::fragment<wmma::matrix_a, 16, 16, 16, __half,
                                       wmma::row_major> af;
                        wmma::load_matrix_sync(
                            af, pa + (size_t)(warp * 16) * NB + kk, NB);
                        #pragma unroll
                        for (int j = 0; j < 8; ++j) {
                            wmma::fragment<wmma::matrix_b, 16, 16, 16,
                                           __half, wmma::col_major> bf;
                            wmma::load_matrix_sync(
                                bf, pb + (size_t)(j * 16) * NB + kk, NB);
...
template <int N, int MATS>
void launch_coop(torch::Tensor mats, torch::Tensor scratch,
                 torch::Tensor panelh, long long* prof) {
    ...
    __half* hp = reinterpret_cast<__half*>(panelh.data_ptr<at::Half>());
    void* args[] = {&mp, &sp, &hp, &prof};
...
void coop_1024x4(torch::Tensor mats, torch::Tensor scratch,
                 torch::Tensor panelh) {
    launch_coop<1024, 4>(mats, scratch, panelh, nullptr);
}

void coop_1024x4_profile(torch::Tensor mats, torch::Tensor scratch,
                         torch::Tensor panelh, torch::Tensor prof) {
    launch_coop<1024, 4>(mats, scratch, panelh,
...
def _scratch_for(mats, sub=4, mt=32, nb=128):
    batch, n, _ = mats.shape
    key = (batch, n, mats.device.index)
    bufs = _SCRATCH.get(key)
    if bufs is None:
        scratch = torch.empty(
            batch * (sub * mt * mt + nb * nb),
            device=mats.device,
            dtype=torch.float32,
        )
        panelh = torch.empty(
            batch * n * nb, device=mats.device, dtype=torch.float16
        )
        bufs = (scratch, panelh)
        _SCRATCH[key] = bufs
    return bufs
...
def mid_probe():
    ...
    work = spd.clone()
    scratch, panelh = _scratch_for(work)
    rows.append({
        "name": "bare coop kernel (in-place, garbage after iter 1)",
        "us": _time(lambda: _COOP.coop_1024x4(work, scratch, panelh)),
        "ok": True,
    })
    ...
    for _ in range(7):
        ...
        _COOP.coop_1024x4_profile(work, scratch, panelh, prof)
        torch.cuda.synchronize()
