"""Experiment 050 V1: single cooperative NB=128 tensor-core Cholesky, 4x1024.
...
constexpr int SMEM_FLOATS = NB * LDS;            // 128*132 = 16896
...
__global__ void coop_cholesky(float* mats, float* dinvg, long long* prof) {
    ...
    for (int kt = 0; kt < N / NB; ++kt) {
        ...
        if (blockIdx.x < MATS) {
            ...
            float (*ds)[LDS] = reinterpret_cast<float (*)[LDS]>(smem);
            ...
            for (int s = 0; s < SUB; ++s) {
                const int o = s * MT;
                // (1) warp 0: register rank-2 factorization of the 32x32
                //     sub-diagonal (exp 039/048-proven mechanism).
                if (warp == 0) {
                    float row_values[32];
                    #pragma unroll
                    for (int c = 0; c < 32; ++c)
                        row_values[c] = ds[o + lane][o + c];
                    #pragma unroll
                    for (int it = 0; it < 16; ++it) {
                        const int k = 2 * it;
                        const int q = k + 1;
                        float inv0 = lane == k ? rsqrtf(row_values[k]) : 0.0f;
                        inv0 = __shfl_sync(0xffffffffu, inv0, k);
                        if (lane >= k) row_values[k] *= inv0;
                        pivot0[lane] = row_values[k];
                        __syncwarp();
                        if (lane >= q)
                            row_values[q] = fmaf(
                                -row_values[k], pivot0[q], row_values[q]);
                        float inv1 = lane == q ? rsqrtf(row_values[q]) : 0.0f;
                        inv1 = __shfl_sync(0xffffffffu, inv1, q);
                        if (lane >= q) row_values[q] *= inv1;
                        pivot1[lane] = row_values[q];
                        __syncwarp();
                        if (lane > q) {
                            const float s0 = row_values[k];
                            const float s1 = row_values[q];
                            #pragma unroll
                            for (int c = 0; c < 32; ++c) {
                                if (c > q && c <= lane) {
                                    float v = fmaf(
                                        -s0, pivot0[c], row_values[c]);
                                    row_values[c] = fmaf(-s1, pivot1[c], v);
                                }
                            }
                        }
                    }
                    ...
                    for (int c = 0; c < 32; ++c)
                        ds[o + lane][o + c] =
                            c <= lane ? row_values[c] : 0.0f;
                }
                __syncthreads();
                // (2) warp 0: exact fp32 inverse of the factored 32x32
                //     lower triangle; lane owns column `lane` of L^-1.
                if (warp == 0) {
                    float x[32];
                    for (int r = 0; r < 32; ++r) {
                        float v = (r == lane) ? 1.0f : 0.0f;
                        for (int p = lane; p < r; ++p)
                            v = fmaf(-ds[o + r][o + p], x[p], v);
                        x[r] = (r < lane) ? 0.0f : v / ds[o + r][o + r];
                        dinv_sh[r][lane] = x[r];
                    }
                }
                __syncthreads();
                ...
                if (rr > 0) {
                    const int o2 = o + MT;
                    // (3) sub-panel solve rows below, fp32: X = A * Dinv^T.
                    float tmp[(96 * 32 + THREADS - 1) / THREADS];
                    int cnt = 0;
                    for (int idx = tid; idx < rr * MT; idx += THREADS) {
                        const int r = idx >> 5, c = idx & 31;
                        float v = 0.0f;
                        for (int p = 0; p <= c; ++p)
                            v = fmaf(ds[o2 + r][o + p], dinv_sh[c][p], v);
                        tmp[cnt++] = v;
                    }
                    __syncthreads();
                    cnt = 0;
                    for (int idx = tid; idx < rr * MT; idx += THREADS)
                        ds[o2 + (idx >> 5)][o + (idx & 31)] = tmp[cnt++];
                    __syncthreads();
