...
if _HAVE_TRITON:
    ...
    _SPLIT32_SHAPES = {
        ...
        (60, 1024): ("tf32", "tf32", 128, "graph", False),
    ...
    def _split32_factor(data: torch.Tensor) -> torch.Tensor:
        ...
        if entry is None:
            try:
                # exp 065: capture the *eager* dataflow (first-touch reads from
                # a static source, everything written to a separate output)
                # rather than the in-place form. Only 640x512 and 60x1024 ever
                # reach this function -- every other _SPLIT32_SHAPES entry is
                # shadowed by the exp-062 gate or a dedicated CUDA kernel -- and
                # both live shapes run eager, so the in-place variant captured
                # here had no live caller and replayed non-finite.
                static_src = torch.empty_like(data)
                static_src.copy_(data)
                ...
                entry = (graph, work, dinv, static_src)
        ...
        if entry is False:
            ...
            return work
        graph, work, _dinv, static_src = entry
        static_src.copy_(data)
        graph.replay()
        return work.clone()
