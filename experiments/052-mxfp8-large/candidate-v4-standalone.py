...
if _HAVE_TRITON:
    ...
    @triton.jit
    def _mx_quant_e4m3_blocked_clipfix_kernel(
        x_ptr,
        q_ptr,
        s_ptr,
        stride_xm,
        ...
        """MXFP8 quantization with a scale bump only for would-clip blocks."""
        pid_m = tl.program_id(0)
        pid_k = tl.program_id(1)
        rows = pid_m * BLOCK_M + tl.arange(0, BLOCK_M)
        cols = pid_k * BLOCK_K + tl.arange(0, BLOCK_K)
...
# ---------------------------------------------------------------------------
_MXFP8_HITS = 0
_MXFP8_CLIPFIX_HITS = 0
_MXFP8_TF32_TAIL_HITS = 0
_MXFP8_ERROR = None
_MXFP8_PTX = None
...
def _mx_quant_e4m3_blocked_clipfix(x: torch.Tensor):
    """V4 quantizer used only by the 16384/nb=2048 MXFP8 route."""
    rows, columns = x.shape
    q = torch.empty(rows, columns, dtype=torch.float8_e4m3fn, device=x.device)
    s = torch.empty(
        rows * (columns // 32), dtype=torch.uint8, device=x.device
    )
    _mx_quant_e4m3_blocked_clipfix_kernel[
        (rows // _MX_QUANT_BLOCK_M, columns // _MX_QUANT_BLOCK_K)
    ](
        x,
        q,
...
def _mxfp8_panel_update(
    ...
    global _MXFP8_HITS, _MXFP8_CLIPFIX_HITS, _MXFP8_TF32_TAIL_HITS
    ...
    is_16384_route = n_rows == 2048
    if is_16384_route and k_cols >= 8192:
        out.addmm_(
            lhs,
            rhs.transpose(-1, -2),
            beta=1.0,
        ...
        _MXFP8_TF32_TAIL_HITS += 1
        return
    if is_16384_route:
        q_lhs, s_lhs = _mx_quant_e4m3_blocked_clipfix(lhs)
        q_rhs, s_rhs = _mx_quant_e4m3_blocked_clipfix(rhs)
        _MXFP8_CLIPFIX_HITS += 1
    else:
...
_LARGE_CFG = {
    ...
    16384: dict(nb=2048, panel_mode="mxfp8", diag_mode="tf32", rec_inv=True, shadow=False),
